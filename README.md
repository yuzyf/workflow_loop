...
```bash
curl -fsSL https://github.com/yuzyf/workflow_loop/releases/download/v0.3.3/install.sh | bash
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "irm https://github.com/yuzyf/workflow_loop/releases/download/v0.3.3/install.ps1 | iex"
```

安装命令固定读取 `v0.3.3` 正式发布中的脚本；不会跟随内容可能变化的 `latest`（最新版本）地址。
...
项目维护者确认当前仓库内容可以直接发布后，在仓库根目录执行下面的命令，并把 `0.3.3` 替换成要发布的新版本号：

```bash
uv run python scripts/release.py 0.3.3
```
