...
- **工作记录表**：程序在环节开始时生成机器可校验的填空表并预填固定事实，AI 只填空位；正式文档由程序按表生成，门禁检查表并检测文档被直接修改，消除格式反复。
...
```bash
curl -fsSL https://github.com/yuzyf/workflow_loop/releases/download/v0.3.5/install.sh | bash
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "irm https://github.com/yuzyf/workflow_loop/releases/download/v0.3.5/install.ps1 | iex"
```

安装命令固定读取 `v0.3.5` 正式发布中的脚本；不会跟随内容可能变化的 `latest`（最新版本）地址。
...
| `workflow scaffold` | 为当前环节生成或补齐工作记录表；已填内容不覆盖 |
...
项目维护者确认当前仓库内容可以直接发布后，在仓库根目录执行下面的命令，并把 `0.3.5` 替换成要发布的新版本号：

```bash
uv run python scripts/release.py 0.3.5
```
