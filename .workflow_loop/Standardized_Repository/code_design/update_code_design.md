...
在代码实施、QA 测试验证、主题验收、最终全量回归和整体验收都完成后，核对产品设计、代码计划、现有架构说明和真实代码，把已经验证的真实结构写回 `spec/代码架构设计.md`，让维护者看到每个产品功能怎样落到实际代码。从零开发没有既有架构文档时，本阶段根据真实实现创建它。
...
- 已确认的产品总说明和功能文档。
- 当前架构设计文档中的真实内容和差异记录；文件不存在时明确记录“本轮从真实实现建立”。
- 实际代码、真实文件路径、类、函数、类型和接口。
- 实施记录中的代码计划、实施动作和代码结果，以及 QA 测试结果、主题验收结果、最终全量回归状态和整体验收结果。
- 需求交付追踪表中当前工作流的全部阶段结果。

## 一、先判断发现的差异属于哪一类

必须先逐项比较产品要求、功能文档、代码计划、已有架构和真实代码，不能直接从代码目录改文档。代码计划负责说明实施前决定，本阶段负责把最终事实写成架构文档；已有架构文档缺失时，按项目实际情况补齐，不把计划内容冒充为现状。
...
- 用实际代码核对代码计划中的实现决定；与真实实现不一致的架构内容要改成真实代码位置和逻辑，不能只写“已完成”。
...
启用工作记录表时，AI 只填两类事实，不手写正式架构的固定结论：

- 最终同步表保留“核对项、核对结论、设计影响、代码影响”四列。六个核对项依次为产品设计核对、功能文档核对、代码实现核对、功能到代码映射、未处理差异、本次同步类型；结论分别为一致、一致、一致、完整、暂无，以及架构变化或架构未变化。
- 设计影响填“需要修改”或“无需修改”；代码影响只有确认无需继续改代码时才填“无需修改”。存在差异时先返回对应阶段，不能提前填通过值。
- 同步说明填写真实核对依据和文档链接。工作流编号、全部有效机器记录编号由程序读取当前状态，不手填旧编号。
- 可选架构正文变更表包含“章节、原文、新文、依据”。章节写正式架构中唯一的完整标题，原文必须在该章节唯一匹配，新文只改已核实事实，依据写真实代码或验收位置；不改标题、表格结构和定位锚点。无需改正文时留空数组。
- 程序先核对全部前置验收、当前回归证据、正文定位与已有手改，再同时生成内部记录和正式架构。任一检查不通过时保留正式架构原文；重复生成使用首次正文基准，不重复追加或替换。

未启用工作记录表的旧轮次仍按模板填写。最终架构文档必须包含“最终同步结论”章节，并记录：
