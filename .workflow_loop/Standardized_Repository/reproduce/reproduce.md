...
启用工作记录表时只填写表，不手写正式文档固定标题或表格。新轮次表版本 3 把根因说明、根因位置、根因证据分列，把运行环境、真实输入分栏，每栏直接写真实事实，程序负责生成标签。旧轮次继续使用开工冻结的版本和原栏目，按本表填写说明保留所需标签，不换表、不丢弃已填内容。

“修复仍存在的不确定性”确实没有内容时整栏只填一条“暂无”，不能留空或混入占位词；其他必需事实仍须逐栏填写。生成路径和内容指纹由程序保存并核对，填写者不改程序专用字段。

1. 同轮可以修复多个缺陷；为每个已确认缺陷分别建主题表、填写复现事实并生成独立缺陷记录。使用 `workflow scaffold --topic <主题一> --topic <主题二>`（按主题补建当前阶段工作记录表）时，程序分别保存各表、正式文档和生成凭据，不覆盖其他缺陷。缺陷记录的标题和正文保留完整中文显示名称，文件名使用程序保存的稳定中文标识。
2. 更新 `bug/索引.md` 并链接本次缺陷记录。
3. 一份缺陷记录只写一个验收主题；旧单表保留原路径、栏目和凭据，与新主题表共存。仅无主题且完全未填的初始占位表可忽略；重复主题、缺表、文件标识冲突和程序字段篡改必须先处理，不能覆盖正式事实。
4. 缺陷复现阶段只填写原始事实，不提前填写后续修复和验收结果。后续逐主题测试与验收，退回只失效直接受影响主题；全部主题完成后共用本轮最终全量回归和整体验收。
