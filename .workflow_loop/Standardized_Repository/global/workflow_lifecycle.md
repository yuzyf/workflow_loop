...
验收计划确认后，固定顺序是：实施计划和实施代码 -> 测试计划 -> 测试代码 -> 正式主题测试。实施阶段只依据产品设计、代码设计和验收条件，不依赖尚未生成的测试计划或 `TC`（测试项编号）；测试计划必须根据已确认实施记录和真实代码填写。

最终全量回归分两步：先执行 `workflow gate regression_test --discuss-done`，再执行 `workflow gate regression_test`。第二条命令会自动运行项目统一测试入口。不存在 `workflow regression` 命令；`workflow test run` 只运行主题测试，不能用于最终全量回归。
