...
本阶段不修改需求、产品规则、验收主题或验收条件，不运行已经由 `qa`（测试验证）完成的自动化测试，也不允许“遗留”“部分通过”或“带条件通过”。

已引入工作记录表的轮次中，AI 在程序生成的 acceptance_result 工作记录表里逐条填写验收结论（passed/failed/blocked）、实际观察结果、证据和人工验收字段；程序按表生成验收结果文档。详见工作记录表与正式文档生成功能。
...
不能只写“自动化测试已通过”，不能写“见测试结果文档”，也不能引用已经因为代码变化而失效的旧记录。找不到当前有效记录时，说明缺什么并返回 `qa`，保留仍有效范围和测试代码后重新登记、执行受影响测试，不用旧编号凑数。
...
验收记录不要求用户提供姓名或角色。AI 不得从系统用户名、Git 用户名或聊天账号推断验收人身份，也不得把这类推断写入正式结果或机器状态。
...
- 实现不符合验收条件：返回 `impl`，修正并确认代码结果后重新进入 `qa`。
- 测试范围、测试代码、任务登记或结果有误：返回 `qa`，按具体变化源回到范围、测试代码、任务或结果步骤，不重做仍有效内容。
- 测试没有执行、执行结果失效或临时环境阻塞：留在或返回 `qa`，只重新登记、执行受影响测试。
