...
- 使用真实文件路径和真实符号名称；没有实现的内容不能写进当前架构。
...
表模式下，已有架构是正文基准；需要更新的事实填入 `architecture_changes`（架构正文变更表）的章节、原文、新文和依据四列，无变化时留空。最终同步表只填写核对结论和同步说明，程序从当前状态取得工作流编号与完整有效机器记录集合，生成第 9 章固定字段和核对清单。程序在写入前检查正文唯一定位、前置验收和真实代码映射，保留其余正文、标题、表格与锚点；手改冲突时不覆盖。旧轮次未启用表时仍使用下面的文档模板。
...
说明本文帮助维护者理解什么，以及本文描述的当前实现经过哪些核对。同步时要核对产品文档、功能文档、架构设计和真实代码是否一致。
...
说明关键结论分别经过运行、测试、代码、文档或用户中的哪种方式确认；仍未确认或存在冲突的内容列入差异，不冒充当前实现。
...
列出产品文档、功能文档、代码、测试和运行结果之间仍存在的差异。不能用代码现状反过来制造新的产品要求；如果发现用户可见功能、规则、边界、使用过程或异常结果变化，必须返回产品设计阶段处理。
...
`update_code_design`（最终代码设计更新）阶段填写以下字段：
