...
- 条件与触发（兼容摘要）：<用一句话概括开始前状态和触发动作>
- 预期结果（兼容摘要）：<用一句话概括可检查结果和通过边界>
...
| 下游 | `impl/<主题文件标识>_实施记录.md`（待生成） | 下一阶段先确认实施计划并完成代码实施 |
| 下游 | `qa/<主题文件标识>_测试计划.md`（待生成） | 实施确认后，基于真实代码编写测试计划 |
```
...
- 每条验收条件必须写清开始前状态、触发动作、可检查结果、通过标准、不通过标准和具体产品设计或缺陷依据；旧的“条件与触发”“预期结果”只作为兼容摘要，不能替代这些字段。
