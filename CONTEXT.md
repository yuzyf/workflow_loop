...
- `abort` 回退本次 Run 已经通过回退基线保存的代码和测试文件；本次计划、测试、实施和验收文档保留在原位置作为中止历史；`state.json` 和 Journal 保留；回退失败不能标记中止成功
_Avoid_: 多 state 并行, start 前强制归档旧 state 到 history（当前不做）, 完成后仍拦 start, 靠手删 state 才能重开, 仅用 current_stage 表达 aborted/completed



**Abort Command** (作废命令):
全局命令 `workflow abort`：将**整个进行中**的 Workflow Run 正式中止。程序先检查实施阶段和测试代码阶段保存的真实文件副本，恢复本次 Run 修改的代码文件；计划、测试、实施和验收文档不从当前目录移走，它们保留为本次中止的过程历史。回退只针对已经登记在清单中的文件，不影响工作流开始前未登记的用户修改，也不使用整个项目压缩包。`state.json` 和 Journal 保留恢复文件、清理路径和中止原因。代码恢复和临时副本清理都成功后才把 `run_status` 标为 `aborted`、记结束时间并解除 Active Run Guard；任一步失败不能伪装成中止成功。对已 `completed` / 已 `aborted` / 无 state 的调用应明确报错或提示，不静默空操作装成功。
_Avoid_: 无正式作废只能手删 state, 取消一个主题却 abort 整个 Run, abort 只改状态不回退已登记代码, 把未保存副本的代码声称可以恢复, 回退时覆盖启动前用户修改, 回退失败仍标记 aborted, 用 current_stage=aborted 冒充 Run 生命周期

进行中的 Workflow Run 独占当前工作区的代码修改范围。用户或 AI 不得在同一工作区同时修改与当前需求无关的代码；如果必须并行处理无关工作，应使用独立代码目录。回退不依赖整个项目的压缩包，而是由各个会修改代码的阶段在真正改代码前，按已确认计划保存本阶段将修改的文件内容。这样既能回退本次 Run，又不会复制整个大型项目。
实施阶段必须先通过 `workflow gate impl --prepare-code`（准备实施代码回退基线）：程序从各主题实施文档读取计划修改的真实文件路径，保存文件修改前的内容；计划新增的文件保存“原本不存在”的记录。进入 `test_code` 时，程序把已有测试文件和测试配置的真实内容加入同一份回退清单；测试代码门禁再把本阶段新建的测试文件登记为“原本不存在”。`state.json` 只保存回退清单路径、文件哈希和当前计划文件列表，真实副本保存在 `.workflow_loop/rollback/<workflow_id>/`。准备基线前不得修改实施代码；副本缺失、路径无法确认或哈希不一致时，门禁不能放行。
单个功能取消时不使用这套整轮回退，也不恢复整个工作流的代码；只按该功能重新确认后的实施计划删除或调整该功能代码。只有整个 Run 使用 `workflow abort` 时，才使用各阶段保存的文件副本回退本次 Run 的修改。
回退快照是工作流进行期间的临时文件。`workflow done` 成功后删除本次工作流的快照和回退清单；`workflow abort` 完成代码回退后也删除真实快照和清单。`abort` 回退失败时不能把 Run 标记为 `aborted`，也不能删除快照，必须保持 `active` 供继续处理。代码已经恢复、但快照清理失败时，State Snapshot 记录“代码已恢复”；再次执行 `abort` 只继续清理快照，不能再次恢复代码，以免覆盖用户在第一次恢复后做的新修改。产品设计、验收计划、测试计划、实施记录和结果文档不属于临时代码快照，中止后保留为过程历史。State Snapshot 和 Journal 只记录快照路径、哈希、恢复结果和清理结果，不保留大文件内容。
_Avoid_: 一个工作区混合多个需求的代码修改, 回退时无法区分修改来源, 用文件时间或文件名猜修改归属, 让 abort 误删其他需求的代码, 为大型项目复制整个项目压缩包, 没有保存真实内容只保存哈希却声称可以回退
...
核心详细区不把多个 Stage 的 `abort`（中途作废）出口连接到一条共享红色总线。每个 Stage 都独立展开自己的短作废分支，例如 `FS-02.AB.0` 调用 `workflow abort`、`FS-02.AB.1` 校验并恢复已保存的代码文件、`FS-02.AB.2` 清理临时回退副本并写 `run_status=aborted`/结束时间/Journal、`FS-02.AB.OUT` 表示允许重新开工。计划和结果文档保留为过程历史。宁可重复这些 Run 级规则，也不能用十几条线汇聚到同一通道。
...
项目根下的 `traceability.md`，按 Workflow Run 分段保存整个项目的需求交付历史，是从产品设计或缺陷复现开始，到最终代码设计更新结束的完整链路和唯一人类可读总入口。程序当前状态分别由 State Snapshot 和 `project.json` 保存。每个工作流章节使用“本次工作”说明本次产品需求或缺陷修复；从零开发清场时删除旧文件，其他工作意图追加当前工作流章节，不覆盖历史。
...
每个工作流章节保留“阻塞和退回记录”，使用时间、阶段、主题 / 验收条件、记录类型、原因或用户决定、后续处理六列。这里保存简短事件和关系，详细验收步骤、证据和用户回答仍放在主题验收结果。一个设计修改或缺陷可以对应多个主题，一个主题可以覆盖多条共同产生同一用户结果的来源，但本次每项工作来源必须至少被一个主题覆盖。当前验收计划哈希包含 `acceptance/index.md` 和本次各主题的 `acceptance/<topic>_plan.md`，因此主题关系、展示顺序或验收计划变化都会使后续阶段重新检查；后续阶段更新追踪表不会使已经确认的验收计划失效。
...
`impl` 必须先为当前工作流的全部验收主题完成实施前计划，用户确认全部计划后才能开始任何代码修改。计划确认后，先通过 `workflow gate impl --prepare-code` 保存本阶段计划修改文件的回退副本，再开始实施。没有依赖的主题可以分别实施；有前置主题的主题按 `acceptance/index.md` 已确认、并由 `impl/index.md` 展示的关系等待；某个主题失败只阻塞它和实际依赖它的主题，不自动阻塞其他独立主题。
...
本阶段不重复运行自动化测试，也不要求用户再次确认自动化测试已经证明的内容。纯自动化验收条件直接使用当前有效的主题测试结果判定；混合验收条件只向用户确认尚未完成的人工部分；纯人工验收条件由用户按验收计划实际检查。全部验收条件按照各自方式通过后，汇总产出 `acceptance/<topic>_result.md`。

主题验收结果是一个主题的最终结果：自动化证据来自 `qa/<topic>_result.md`，人工结论只在本阶段记录。程序门禁不能让主题验收绕过自动化测试，也不能要求纯人工主题伪造测试结果；阶段确认时只更新主题验收结果列，并记录 `acceptance_result_hash`。修 bug 在这里更新为“主题验收通过，待全量回归”，不能在测试执行阶段提前关闭缺陷。
...
验收发现问题后，程序不能根据“不通过”自动固定退回 `impl`。AI 必须先继续提问并检查产品设计、验收条件、测试计划、测试结果、实施记录和真实代码，说明问题属于哪一层并给出建议退回阶段；用户确认后，AI 才调用 `workflow return`。产品要求变化返回 `spec`，验收条件问题返回 `acceptance_plan`，测试范围问题返回 `test_plan`，实现问题返回 `impl`，测试代码问题返回 `test_code`，测试执行问题返回 `test_execution`。

`acceptance/<topic>_result.md` 只保存最终验收步骤、实际观察结果、用户最终回答、最终判定和必要证据，不复制完整聊天问答。中间的澄清过程、问题追问和退回过程由 Journal 保存简短历史；发生阶段退回时，`traceability.md` 只记录阻塞或退回事件、原因和后续处理。

验收结果按受影响主题整体失效，不在同一主题内保留部分旧 `AC-xx` 的通过记录。产品设计、验收计划、实施代码或测试结果变化时，程序和用户先确定受影响主题；删除这些主题当前的 `acceptance/<topic>_result.md`，清除 State Snapshot 中这些主题的当前有效验收记录，并把 `traceability.md` 的验收结果列恢复为待验收；Journal 保留旧结果失效的原因和时间。没有受到影响的独立主题继续保留当前有效结果。

主题验收结果的职责分开：程序保存测试记录、用户实际回答、确认时间、证据和当前有效状态；AI 根据这些程序记录和已确认验收计划生成可读的 `acceptance/<topic>_result.md`；门禁逐条检查 Markdown 内容与程序记录一致。AI 不能改写程序记录中的回答、时间或结果，程序也不直接生成只有固定字段、缺少人类可读说明的空文档。

主题验收、最终全量回归和整体验收分别负责不同判断：主题验收逐条确认各主题的 `AC-xx`；最终全量回归确认合并后的代码没有破坏项目其他功能；整体验收由用户确认这些主题合在一起是否完成本次需求。整体验收不重复执行主题验收步骤，也不生成 `overall_result.md`。整体验收不通过时，发现已有验收条件未满足就使受影响主题结果失效并返回对应阶段；提出新产品要求就返回 `spec`；取消一个主题就返回 `spec`；整个需求都不再继续才执行 `workflow abort`。

某个主题的全部 `AC-xx` 通过后，立即生成该主题的 `acceptance/<topic>_result.md`，不等待其他没有依赖关系的主题。存在前置主题时，必须等前置主题通过后才能开始当前主题验收。`acceptance/index.md` 始终保留全部主题的结果入口；当前工作流全部主题都有有效通过结果后，才通过 `topic_acceptance` 阶段并进入最终全量回归。
...
主题验收不存在“遗留”“部分通过”或“带条件通过”。只有全部验收条件都已经完成、能够验证并且结果通过时，才生成正式的 `acceptance/<topic>_result.md`，主题总结果固定为“通过”。任一验收条件未通过、无法验证、缺少证据或受到阻塞时，都不生成可供后续阶段使用的正式结果，也不能进入最终全量回归。

验收失败、无法验证或阻塞不是当前有效的主题验收结果。程序不生成正式结果文件，也不在 State Snapshot 中保留可供后续门禁使用的“通过”记录；Journal 追加保存用户实际回答、观察结果、证据、失败原因和退回阶段，`traceability.md` 的“阻塞和退回记录”只保存简短事件、原因和后续处理。修正后重新验收成功时，State Snapshot 保存新的当前有效通过记录，旧失败过程继续保留在 Journal 中，不被覆盖或伪装成从未发生。

发现问题后必须返回产生问题的阶段：实现不符合要求返回 `impl`；测试代码或测试执行有问题返回 `test_code` 或 `test_execution`；测试计划不能覆盖验收条件返回 `test_plan`；验收条件本身遗漏、矛盾或不能判断返回 `acceptance_plan`；产品要求无法实现、需要删除或改变时返回 `spec` 重新确认产品设计。

验收时用户只取消当前工作流中的一个功能或验收主题，不执行 `workflow abort`，也不恢复整个项目代码。AI 先确认受影响主题，再调用 `workflow return --to spec --topic "<主题>" --reason "用户决定取消这个功能"`。产品设计阶段删除对应功能或产品行为；验收计划阶段重新确认剩余主题；实施阶段按照修改后的产品设计，删除只服务于已取消功能的代码，同时保留其他功能仍然使用的公共代码。随后只重做受影响主题的测试代码、测试执行和主题验收，最后仍执行全量回归。用户决定整个工作流都不再继续时，才执行 `workflow abort`。`bugfix` 未全部通过时不得关闭缺陷。
_Avoid_: 取消一个主题却中止整个 Run, 用整项目快照覆盖其他功能代码, 只删验收主题不删除对应产品设计和代码, 按文件整体回退仍被其他功能使用的公共代码, 取消功能后跳过受影响测试
...
人工验收步骤既要在聊天中展示，也要写入对应 `acceptance/<topic>_result.md`，使后续维护者能够复核用户当时怎样验收。用户明确回答后，程序按当前工作流、验收主题和 `AC-xx` 分别记录用户实际回答、确认结果、确认时间和证据：State Snapshot 保存当前仍有效的验收记录，Journal 追加保存历次回答、失效和重新验收历史。结果文档只能引用 State Snapshot 中当前有效的程序记录，不能由 AI 自行填写“用户已确认”。
...
每个验收主题全部验收条件通过后形成最终文档，路径为 `acceptance/<topic>_result.md`。顶部固定记录工作流编号、验收主题、验收结果和验收完成时间；验收结果固定为“通过”。文档固定包含“验收依据、验收条件结果、上下游文档”三章。
...
全部主题都形成明确为“通过”的正式验收结果后，`regression_test` 直接执行项目配置的统一测试入口。退出码为 `0` 才算通过；程序把执行状态写入 `state.json` 和 `journal.jsonl`，并更新 `traceability.md`。没有测试入口、入口无法启动、测试失败或执行超时，都不能进入整体验收。bugfix 回归失败时把缺陷状态改为“回归失败，重新处理中”。这些固定条件直接写在代码中，不依赖人工填写结果文档。

**Overall Acceptance Stage** (`overall_acceptance`):
最终全量回归通过后，由用户确认整个需求是否完成。程序门禁重新检查当前工作流的全部主题验收结果和最终全量回归都已经通过；第三道门记录用户是否明确接受整个需求的交付结果。该阶段不生成 `acceptance/overall_result.md`，也不建立只重复固定门禁的独立规范文件。用户确认结果写入 State Snapshot、Journal 和 `traceability.md`。任一前置结果或用户确认不满足时，不能进入详细代码设计更新；用户决定不再继续时中止当前工作流，不能以未完成状态收工。
...
| 测试计划 | `test_plan` | 测试计划重新确认；实施和测试代码先复核，测试结果重新执行 |
| 实施代码或实施记录 | `impl` | 实施计划和现有代码重新核对；测试代码和全部结果重新确认 |
| 测试代码、测试配置或统一测试入口 | `test_code` | 测试代码重新核对；全部主题测试结果重新执行 |
| 主题测试结果 | `topic_acceptance` | 重新主题验收；全量回归和整体验收重新确认 |
