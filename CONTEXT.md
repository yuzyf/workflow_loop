...
产品设计、验收计划、测试计划、实施计划、实施记录、测试结果、验收结果和最终代码设计文档都必须提供局部上下游链接。每份文档只直接链接当前内容的上游依据、下一阶段文档和项目根 `traceability.md`，不复制整条链路和执行状态。上游链接让读者返回需求与依据；下游链接让读者继续查看下一阶段怎样承接；完整状态和跨阶段跳转统一从 `traceability.md` 查看。局部文档使用可预先确定的稳定路径，后续文件生成后链接自动可用，不通过修改已确认的上游正文补链接。
_Avoid_: 只能从全局表打开局部文档, 局部文档没有来源和去向, 每份文档复制一套状态造成不一致, 后续阶段为补链接改动已确认内容并触发哈希失效
