...
| 核对项 | 当前事实 | 依据 |
|---|---|---|
| 当前版本身份 | 源码、命令身份、六个维护脚本、README、项目安装标记和自动发布配置统一为 `0.3.1` | `pyproject.toml`、`src/workflow_loop/__init__.py`、`install.sh`、`install.ps1`、`update.sh`、`update.ps1`、`uninstall.sh`、`uninstall.ps1`、`README.md`、`.workflow_loop/project.json`和 `.github/workflows/release.yml` |
...
| 最近保存的公开发布证据 | `0.2.0` 的标签、GitHub Actions 任务、PyPI 页面和 GitHub 正式发布页是已完成历史证据；当前 `0.3.1` 身份是现行源码和发布配置事实，不用旧任务编号冒充新版本已发布证明 | [0.2.0 主题测试结果](../qa/0_2_0_在_GitHub_和_PyPI_完成正式发布_测试结果.md)、`tests/release_publication_checks.py`和当前版本身份文件 |
...
| 最近已完成的交付证据 | 历史发布维护轮次的主题验收、最终全量回归和整体验收已通过；当前轮次的交付状态只从当前工作流状态读取 | [当前仓库可由一条命令直接发布为新版本_验收结果](../acceptance/当前仓库可由一条命令直接发布为新版本_验收结果.md)、[对应测试结果](../qa/当前仓库可由一条命令直接发布为新版本_测试结果.md)和本文第 9 节 |
...
2. 安装器检查 Python 3.11 或更高版本、现有命令身份和完整写入范围；用户确认后，安装 `0.3.1` 的全局命令与项目骨架。
...
公开使用入口是仓库根目录的中文 `README.md`（项目说明文档）。当前 README 展示四个公开状态徽章，说明产品定位、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台正式安装命令、最小使用示例、简洁命令表、使用 `uv`（Python 项目管理工具）的源码开发方式、仓库结构和详细设计入口。README 的安装入口和本地版本身份均为 `0.3.1`；无需开发任务规则同步到安装器生成的 `AGENTS.md`（AI 契约文件）和 README。
...
| 产品规则 | 代码职责 | 实际承担位置 |
|---|---|---|
| 当前版本一致、跨平台、事务安装 | 终端适配与共同安装事务分离；所有当前安装身份均为 `0.3.1` | `install.sh`、`install.ps1`、`src/workflow_loop/installer.py` |
...
| 公开说明、包元数据和当前版本发布 | 版本常量、静态说明、许可证、打包配置和发布任务共同承载 `0.3.1`；发布任务按 `v0.3.1`（版本 0.3.1 标签）先验证、再发布 PyPI、最后创建 GitHub 正式发布页 | `src/workflow_loop/__init__.py`（产品版本常量）、`README.md`（项目说明文档）、`LICENSE`（许可证文件）、`pyproject.toml`（Python 打包配置）、`.github/workflows/release.yml`（自动发布配置） |
...
| 计划、真实差异和实施记录一致 | 程序双向比较代码计划、真实差异和实施记录三组文件路径，分别报告计划了但未修改、修改了但未计划、修改了但未记录、记录了但无真实差异 | `src/workflow_loop/rollback.py::validate_implementation_changes`、`src/workflow_loop/stages/stages.py::ImplStage` |
...
| 核心快照只比较登记路径 | 核心代码只取现状架构基线、确认代码计划和代码结果登记的路径；测试只取测试验证环节内测试计划登记的测试文件、统一入口和明确登记配置；未登记生成物、依赖、构建、缓存和运行数据忽略 | `src/workflow_loop/snapshots.py::collect_snapshot`、`src/workflow_loop/verification.py::inspect_invalidation`、`src/workflow_loop/rollback.py::validate_implementation_changes` |
...
`README.md`（项目说明文档）和 `LICENSE`（MIT 许可证文件）属于同一公开说明与发布适配层，不被运行时命令调用。`pyproject.toml`（Python 打包配置）提供 README、作者 `yuzyf`、MIT 许可证、固定项目简介和仓库链接等分发元数据，并使说明与许可证进入构建产物。当前版本身份为 `0.3.1`；自动发布层负责版本核对、全量测试、构建、三平台安装验证、PyPI 发布和 GitHub 正式发布页创建，历史发布证据留在对应测试与验收记录中。
...
`src/workflow_loop/rollback.py` 保存首次原内容并负责恢复，同时比较代码计划、真实差异和代码结果三组文件。`src/workflow_loop/test_mapping.py` 连接主题、测试项、验收条件和测试函数。`QaStage`（测试验证环节策略）统一用户可见环节，但保留各事实来源的独立边界：测试计划说明准备怎样验证，`src/workflow_loop/test_execution.py` 保存待执行任务和逐测试项机器记录，测试结果文档整理已经发生的结果。`src/workflow_loop/process_runner.py` 受控运行子进程。`src/workflow_loop/test_report.py`（结构化测试报告解析模块）适配项目已登记的测试框架，返回真实目标名称和执行统计；`src/workflow_loop/acceptance_records.py` 把当前机器记录或用户实际回答固化为验收记录。
...
- **对应产品内容**：公开产品身份、项目简介、作者、MIT 许可证和仓库链接保持一致；当前源码、README、安装脚本、分发元数据和发布任务统一使用 `0.3.1`，已发布版本不覆盖或重发。
...
- **上游**：维护者完成版本号、说明、许可证、元数据和发布配置，把最终提交推送到主分支，再让 `v0.3.1`（版本 0.3.1 标签）指向该提交；GitHub Actions（GitHub 自动任务）读取标签并启动发布流程。
...
- **下游**：用户从 README 复制当前系统的安装命令；安装脚本从 `v0.3.1` 正式发布页取得脚本，并从 PyPI 安装 `workflow-loop==0.3.1`（明确版本的 Python 分发包）。
- **状态和数据**：当前发布配置使用 `v0.3.1`（版本 0.3.1 标签）、`0.3.1` 项目包版本、名称为 `Workflow Loop 0.3.1` 的正式发布、固定发布说明和六个维护脚本。历史版本的提交、任务编号和公开页面证据留在对应测试与验收记录中。
...
- **对应产品内容**：所有验收行为都写明前提、触发、可检查结果和通过/不通过边界；代码计划、真实差异和实施记录完全对应；测试验证在实施后使用真实入口、动作和观察位置；正式测试必须真正运行登记目标。
...
- **主要处理**：验收解析器拒绝缺字段和固定占位词。实施门禁对比“计划路径集合 = 真实差异路径集合 = 实施记录路径集合”；显式接受既有代码时因没有可信修改前差异，只核对计划与记录、路径存在性和用户确认，不伪称已完成三方比较。测试计划解析器仍保存产品入口、代码入口、测试入口、准备数据、执行动作、观察位置、预期结果、不通过表现和证据要求，与当前验收条件及实施记录逐项对应，但这些内容属于 `qa`（测试验证）内部计划事实，不再作为独立用户可见阶段。
...
- **第二与第三道门**：`cmd_gate()`（执行阶段门禁函数）在第二道门调用本阶段全部固定校验，成功后把材料指纹、责任输入哈希、报告哈希和阶段结果哈希写入 `verification_credential`（第二道门校验凭据）。第三道门先重新计算同一组轻量指纹；全部相等时只取得用户确认并推进，任一变化时清除旧凭据并回到第二道门。`qa`（测试验证）和最终回归的真实测试只在第二道门规定的执行点运行一次，第三道门绝不重跑。
...
- **对应产品内容**：实施快照只覆盖现状架构基线、用户确认的代码计划和代码结果三者登记的核心代码路径；测试快照只覆盖 `qa`（测试验证）内部测试计划登记的测试文件、项目统一测试入口和明确登记的测试配置。代码计划、代码结果、测试计划、测试代码和测试结果分别保留哈希与失效边界，不能用一个阶段总哈希掩盖具体变化。未登记的文件即使发生变化也不触发本轮失效。
...
- **登记范围**：代码候选集是 `D_code`（现状架构基线登记路径）、`P_impl`（用户确认代码计划登记路径）和 `R_impl`（最终代码结果登记路径）的并集；三组集合先做一致性校验，缺项、多项和只在一组出现的路径分别报告，不能静默扩大或缩小快照。集合不一致时只生成诊断，不建立可通过的聚合哈希。测试候选集是 `P_test`（测试验证内部测试计划登记的测试文件）、`E_test`（统一测试入口登记的入口文件）和 `C_test`（明确登记的测试配置文件）的并集，也先做登记一致性校验。最终只对这些明确列出的项目相对普通文件取快照；路径不存在、越界、符号链接、目录或被登记为生成物/依赖结果时报告具体范围错误。程序不通过 Git 状态、文件后缀或全仓递归扫描推断范围。
- **快照事实**：每个登记路径保存 `path`（路径）、`exists`（是否存在）、`file_type`（文件类型）和 `content_hash`（内容哈希）；按规范化路径排序后，用这四个字段的规范化序列计算聚合哈希。`impl_plan_hash`（代码计划哈希）、`impl_result_hash`（代码结果哈希）、`qa_scope_hash`（测试范围指纹）、`qa_plan_hash`（测试计划哈希）、`qa_code_hash`（测试代码哈希）和 `qa_result_hash`（测试结果哈希）分别绑定各自负责的文档或登记快照。代码计划变化要求重新确认实施范围；代码结果或产品代码变化使 `qa` 失效；测试计划变化使测试代码、任务、结果和后续失效；测试代码变化使任务、结果和后续失效；只有结果文档中的机器固定字段与当前记录不一致时重新生成结果，不反向清除仍有效的测试代码。对已登记路径严格使用四种差异标签：`added`（由不存在变为存在）、`modified`（文件内容哈希改变）、`deleted`（由存在变为不存在）和 `type_changed`（文件类型改变）。没有逐文件基线时只能报告“未检查：缺少逐文件基线”，不能用一个旧聚合哈希猜测变化来源。
...
- **共享配置拆分**：`verification.py` 只对 `pyproject.toml`（Python 项目配置）、`package.json`（Node.js 项目配置）和 `setup.cfg`（Python 工具配置）做结构化拆分，分别计算测试专用部分和产品部分的快照。任一文件解析失败，或者是其它共享配置文件时，整份文件同时计入测试与产品两侧，避免把无法安全分类的变化当成无关变化。
...
产品要求见[功能_安装到项目.md](./功能_安装到项目.md)。当前安装使用现有 Python 3.11 或更高版本、目标版本 `0.3.1`、一次确认和完整事务；取消或失败不能留下半套项目。用户先从公开 README 了解产品和环境要求，再使用当前系统的正式安装入口。
...
| 骨架身份检查 | `src/workflow_loop/project.py::check_skeleton` | 核对产品版本、项目状态、两套资源仓库和智能体契约；当前产品版本为 `0.3.1` | 残缺或版本异常时写入前停止 | `tests/test_project.py::test_is_installed_rejects_project_state_without_complete_skeleton` |
...
| 公开说明入口 | `src/workflow_loop/__init__.py::PRODUCT_IDENTITY`（命令版本身份）；`README.md`（根目录项目说明文档） | README 展示四个徽章、统一产品身份、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台命令、最小示例、简洁命令表、`uv`（Python 项目管理工具）源码开发方式、仓库结构和详细文档链接；版本身份与安装命令中的 `0.3.1` 保持一致 | 文件缺失、结构不完整、出现旧仓库当前链接或命令版本不一致时，公开项目测试失败 | `tests/test_public_project.py::test_readme_has_the_confirmed_chinese_structure_and_entries`、`tests/test_light_task.py::test_installed_contract_and_readme_describe_the_light_task_rules` |
...
| 三道门推进 | `src/workflow_loop/cli.py::cmd_gate` | 第一门只确认讨论材料；第二门完整执行阶段校验并保存校验凭据；第三门只复核凭据绑定输入后取得用户确认并推进 | 跳步、材料变化、凭据缺失或输入漂移时回到第二门；第三门不重复完整校验或测试 | `tests/test_commands.py::test_three_gates_advance_in_the_required_order` |
...
产品要求见[功能_实施并保护项目修改.md](./功能_实施并保护项目修改.md)。全部主题的实施只依据已确认的产品设计、现状架构基线（如有）、验收计划和穿刺结论，不要求最终代码设计已先完成，也不依赖尚未制定的测试计划或 `TC`（测试项）。`impl`（代码实施）内部固定为“代码计划→代码实施→代码结果”：用户只确认代码计划和代码结果，中间不增加门禁。实施内容先保存真实原内容；同一环节调整只能补充尚未修改的新路径。
...
| 实施记录门禁 | `src/workflow_loop/stages/stages.py::ImplStage` | `impl` 内部按“代码计划→代码实施→代码结果”连续完成；第二道门完整检查计划、真实差异、实施记录和开发检查，第三道门只复核第二道门保存的校验凭据 | 主题缺失、计划外修改、有未解决问题或验收依据不可导航时失败；复用既有代码时保存复用依据而不制造无意义修改 | `tests/test_current_workflow_contracts.py::test_impl_gate_blocks_every_detected_mismatch` |
...
| 实施三组路径一致 | `src/workflow_loop/rollback.py::validate_implementation_changes`（校验实施文件变化函数） | 普通实施门禁要求计划文件集合、基线后真实差异集合和代码结果集合完全相等；接受既有代码的显式例外只检查计划与记录、路径存在性和用户确认 | 分别报告计划未改、计划外修改、修改未记录和记录无差异；不用一个“不一致”隐藏原因 | `tests/test_rollback.py::test_planned_actual_and_recorded_implementation_paths_must_match` |
...
产品要求见[功能_编写并执行测试.md](./功能_编写并执行测试.md)。本节描述单一用户可见的 `qa`（测试验证）环节。环节开始时用户确认一次测试范围和通过标准；随后 AI 与程序按“计划检查→复用或修改测试代码→冻结测试代码清单→登记任务→正式执行→生成结果”的内部顺序连续推进，正常路径不在内部步骤之间增加门禁。测试代码必须追踪到主题、测试项、验收条件和产品入口，并在准备、执行、验证三步中真正调用产品入口。正式结果除了进程退出码，还必须保存结构化报告中的真实目标名称、执行数、跳过数、失败数和错误数；环节结束时用户只确认一次最终结果。

| 场景 | 代码位置 | 关键逻辑与状态 | 异常结果 | 验证位置 |
|---|---|---|---|---|
| 测试标识校验 | `src/workflow_loop/test_mapping.py::validate_workflow_test_markers` | 在 `qa` 内部核对测试文件附近的主题、测试项、验收条件、方法、产品入口、准备内容、执行动作、关键断言和预期证据，与测试计划逐项一致 | 数据字符串中的伪标识、一函数绑定多项或任一关键字段不一致时留在 `qa`；程序不尝试跨语言证明函数调用语义 | `tests/test_current_workflow_contracts.py::test_test_code_calls_product_entry_and_asserts_observable_result` |
...
| 测试代码冻结与门禁凭据 | `src/workflow_loop/state.py::StageState`（单个研发环节状态类型）和 `src/workflow_loop/stages/stages.py::QaStage`（测试验证环节策略） | 保存用户确认的测试范围指纹、测试代码清单哈希和第二道门完整校验凭据；第三道门只核对这些凭据绑定的输入指纹，不重复完整检查 | 凭据缺失、范围或文件漂移、报告记录不匹配时清除 `qa` 后续结果并停留当前环节 | `tests/test_current_workflow_qa.py::test_gate_ac01_third_gate_reuses_second_gate_credential` |
...
| 逐项机器记录与结果整理 | `src/workflow_loop/test_execution.py::run_prepared_tasks`；`src/workflow_loop/test_report.py::parse_test_report`（解析结构化测试报告函数）；`src/workflow_loop/acceptance_records.py` | 执行前删除旧报告，按主题和测试依赖运行后读取新报告，从具体用例重算计数，并要求实际目标集合与登记集合完全相等；程序根据当前机器记录生成或回填测试结果文档，纯人工主题只核对测试计划和人工交接状态 | 前置失败时停止依赖项；报告缺失、目标缺少或多出、零执行、有跳过、有失败、格式或计数冲突、适配器未知或退出码非零均清除旧成功；纯人工主题不伪造机器结果 | `tests/test_current_workflow_contracts.py::test_structured_report_requires_exact_non_skipped_current_targets` |
...
`verification.py` 只服务完整研发流程，分别计算产品、现状架构基线、验收计划、代码计划、代码结果、测试范围、测试计划、测试代码、测试结果、主题结果和最终回归的当前哈希。内容失效和主动返回共用“先检查、后应用”的规则：`inspect_invalidation()`（检查失效原因函数）只读当前登记快照、阶段状态和追踪关系，收集全部差异并生成带指纹的清零计划；`apply_invalidation()`（应用失效计划函数）再次核对计划指纹后一次性清理目标及下游字段并追加一条流水。任何检查错误、计划漂移或追踪表结构错误都只生成诊断，不先清状态。验收计划变化清 `impl` 和 `qa`；代码结果或产品代码变化清 `qa`；`qa` 范围、测试计划、测试代码、任务登记和机器记录分别只清自身及后续责任范围；测试结果文档机器字段抄写错误时优先由程序重生成，不反向清除有效测试代码。主题范围仍只清除用户确认的直接受影响主题；最终全量回归负责发现跨主题或间接影响。无需开发任务使用讨论时确认的针对性核对方法，不调用该模块制造固定哈希门禁。
...
`rollback.py` 只把三种完整研发轮次的开工、统一测试入口、代码实施和 `qa` 测试代码纳入同一轮首次副本体系；穿刺资产单独留在 `.workflow_loop/spike_tmp/<工作流编号>/<穿刺项标识>/`，只有未形成结论的半成品可在整轮作废时清理。追加路径不得覆盖第一次保存的原内容；正式收工和成功作废删除本轮回退副本，失败作废保留恢复依据和进度。该模块集中取得代码计划、基线后真实差异和代码结果三组路径，使普通实施门禁不会只拦“计划外修改”却放过“计划内没改”。无需开发任务不调用该模块，作废时保留实际状态。
...
`verification.py`（状态有效性检查模块）从当前阶段材料解析第 5.17 节定义的登记路径，交给 `state.py`（单轮状态模块）保存逐文件事实。快照不递归扫描项目、不按后缀推断、不读取 Git 状态；聚合哈希只是已登记事实的确定性摘要。`rollback.py`（实施回退模块）复用代码登记范围比较代码计划、真实差异和代码结果；`test_mapping.py`（测试映射模块）复用 `qa` 的测试计划登记范围；`test_execution.py`（测试执行模块）保存任务和机器记录指纹。依赖安装、构建输出、缓存和运行时文件永远不因目录扫描进入范围；锁文件和构建配置只有在明确登记且通过核心范围校验时才可进入，生成物和依赖结果被登记时报告范围错误。
...
本节记录当前工作流在真实代码、测试、主题验收、最终回归和整体验收完成后的同步结论。
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：[产品总说明](./产品总说明.md)和当前功能文档与真实代码一致；本轮五份[主题验收结果索引](../acceptance/索引.md)使用机器记录 RUN-20260812T022847+0000-417887b5、RUN-20260812T022848+0000-07c1e9f5、RUN-20260812T022848+0000-354c4eb5、RUN-20260812T022848+0000-51024bbe、RUN-20260812T022849+0000-cde0df08、RUN-20260812T022849+0000-7eb0c3fe、RUN-20260812T022849+0000-8ed595a0、RUN-20260812T022850+0000-857d1c59、RUN-20260812T022850+0000-d4c7b55f、RUN-20260812T022850+0000-511fa26a、RUN-20260812T022850+0000-23645be8、RUN-20260812T022850+0000-1e41b5b0、RUN-20260812T022850+0000-219df304、RUN-20260812T022850+0000-e0811fe2、RUN-20260812T022851+0000-be9c8047、RUN-20260812T022851+0000-cfb09781、RUN-20260812T080454+0000-b7c25dff、RUN-20260812T080455+0000-70a800d7、RUN-20260812T080455+0000-4e7726d0、RUN-20260812T080456+0000-a3a349f9、RUN-20260812T080457+0000-2659fcb8、RUN-20260812T080457+0000-93f9ce7f；最终全量回归于 2026-08-12T08:40:30+00:00 至 2026-08-12T08:41:07+00:00 执行 `scripts/test_all.sh`，退出码为 0，397 项测试通过，机器记录为 REG-20260812T084030+0000-b69c5b95；用户已确认全部主题组合后完成本次需求；交付关系见[需求交付追踪表](../需求交付追踪表.md)。

本轮已把完整研发路径、`impl` 三段实施记录、`qa` 单一测试验证、第二道门凭据复用和可复用穿刺资产的真实实现同步到本文。架构变化只涉及工作流编排、状态和验证边界；产品设计与功能文档核对一致。
