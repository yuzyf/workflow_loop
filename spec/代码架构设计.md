...
- **架构与实施位置错误的最小粒度**：`artifact_validation.py::_feature_code_mappings`（解析功能到代码映射函数）报告功能名、代码设计章节、具体表格/行/列、原单元格、预期的项目内相对文件路径和可定位符号；`rollback.py::validate_implementation_changes_report`（实施变化结构化报告函数）对实施记录同一行的文件、类/函数/配置项、实际逻辑、输出和验收条件分别校验。位置只写“组件”等泛称时，报告保留目标文件和原值，并要求改成文件内真实函数名、类名或配置项；同一行其它错误在同一次报告中一并列出。
...
- **为什么是关键节点**：代码计划回答“本轮允许改什么”，实施观察回答“进入实施后实际改了什么”，两者不能使用同一集合。只观察计划登记路径会漏掉未登记源码、测试、脚本或配置变化；扫描整个项目又会把依赖安装结果、构建输出、缓存和正式流程文档误判成实施修改。
...
- **登记责任范围**：代码登记集来自代码设计“代码位置”、用户确认的实施计划、代码结果和测试计划；测试登记集来自测试计划中的测试文件、统一测试入口引用脚本和明确配置。计划、真实差异和结果记录先做集合一致性校验，缺项、多项和只在一组出现的路径逐文件报告。阶段失效只比较当前责任输入，避免无关文件让已通过结果失效；程序不使用 Git 状态推断责任范围。
...
- **冻结与重设**：进入或恢复到 `impl` 时，`ensure_impl_recovery_baseline()`（确保实施恢复基线函数）只在基线不存在时冻结一次；`cmd_discuss()`（列出必读材料命令）第一次成功加载实施材料前通过 `_take_impl_code_baseline_notice()`（取得一次性基线提示函数）说明冻结已经发生、`workflow discuss` 和 Git 提交不会重写它。只有用户明确确认当前工作区应成为新的实施前现状时，AI 才执行 `workflow gate impl --rebaseline`（重设实施基线命令）；普通讨论和重复准备不能覆盖首次原内容。
- **回退与三方核对**：`prepare_impl()` 在保存计划文件首次原内容前比较完整入场快照，入场后提前发生的未登记变化会直接拒绝准备；回退清单保存完整初始文件清单，`changed_paths_since_prepare()`（取得实施后变化路径函数）因此能发现未登记文件的新增、修改和删除。`validate_implementation_changes_report()` 再把完整真实差异与计划集合、代码结果集合逐文件比较；未登记变化按“实际变化在计划外”报告，恢复无法证明完整时不能标记成功。
...
- **失败结果**：登记责任范围非法时，实施或测试门禁报告具体路径并停留当前阶段；完整观察范围发现计划外源码、测试、脚本或配置变化时，报告变化类型、文件路径及恢复原内容或经确认重设基线的处理方法。依赖、构建、缓存和正式流程文档变化既不冒充实施修改，也不会触发错误退回。
...
| 冻结并说明实施入场基线 | `src/workflow_loop/cli.py::_freeze_impl_code_baseline`（冻结实施代码基线函数）；`src/workflow_loop/cli.py::_take_impl_code_baseline_notice`（取得一次性基线提示函数） | 进入或恢复到 `impl` 时同时冻结登记产品快照和完整实施观察快照；第一次成功读取实施材料前只提示一次冻结时机、范围和重设条件 | `workflow discuss` 或 Git 提交不会重写基线；只有用户明确确认后才能用 `workflow gate impl --rebaseline` 重设 | `tests/test_commands.py::test_discuss_announces_impl_baseline_frozen_before_first_material_read` |
| 发现未登记实施变化 | `src/workflow_loop/stages/stages.py::ImplStage.discussion_validate`（实施讨论校验函数）；`src/workflow_loop/verification.py::compute_complete_implementation_file_snapshot`（完整实施观察快照函数） | 计划确认前比较完整观察快照，登记哈希相同也继续检查未登记源码、测试、脚本和配置 | 列出新增、修改或删除的具体路径；要求先恢复遗留变化，或在用户接受当前现状后重设基线 | `tests/test_stages.py::test_impl_discussion_detects_unregistered_file_change_from_complete_baseline` |
...
| 实施三组路径和逐列事实一致 | `src/workflow_loop/rollback.py::validate_implementation_changes_report`（实施变化结构化报告函数） | 要求计划文件集合、完整观察到的真实差异集合和代码结果集合完全相等；同时逐行校验文件、真实符号、逻辑、输出和验收条件 | 每个计划未改、计划外修改、修改未记录、记录无差异和同一记录行的独立格式错误分别报告，不用一个“不一致”隐藏原因 | `tests/test_rollback.py::test_implementation_record_reports_every_invalid_row_fact_at_once`、`tests/test_rollback.py::test_complete_inventory_reports_unregistered_modified_and_new_files` |
...
| 测试代码冻结与门禁凭据 | `src/workflow_loop/state.py::StageState`（单个研发环节状态类型）和 `src/workflow_loop/stages/stages.py::QaStage`（测试验证环节策略） | 保存用户确认的测试范围指纹、测试代码清单哈希和第二道门完整校验凭据；第三道门只核对这些凭据绑定的输入指纹，不重复完整检查 | 明确发现范围、文件或报告记录漂移时清除 `qa` 后续结果；凭据比较本身无法完成时保留已有凭据和结果，报告异常并停留当前环节 | `tests/test_current_workflow_qa.py::test_gate_ac01_third_gate_reuses_second_gate_credential` |
...
`rollback.py`（实施回退模块）把完整实施观察清单保存进首次回退依据，用它取得真实新增、修改和删除集合，再与计划集合和代码结果集合逐文件比较；`test_mapping.py`（测试映射模块）和 `test_execution.py`（测试执行模块）继续只使用 `qa` 的责任登记范围和机器记录指纹。依赖安装、构建输出、缓存、运行时文件和正式流程文档明确排除在完整观察范围外，因此“允许修改的集合”和“必须观察的集合”各自回答一个问题，不再互相替代。
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：[产品总说明](./产品总说明.md)和当前 17 份功能文档与真实代码一致；本轮[主题测试结果](../qa/所有阶段门禁失败时一次指出全部真实原因和改法_测试结果.md)的 TC-01 至 TC-09 均通过，机器记录分别为 RUN-20260814T035842+0000-f1e81e55、RUN-20260814T035842+0000-9a2d4b79、RUN-20260814T035843+0000-81f62a07、RUN-20260814T035843+0000-57bb8d1d、RUN-20260814T035843+0000-74a14822、RUN-20260814T035843+0000-b3bbe518、RUN-20260814T035843+0000-73592ff0、RUN-20260814T035843+0000-d75539c7、RUN-20260814T035843+0000-530e5d37；[主题验收结果](../acceptance/所有阶段门禁失败时一次指出全部真实原因和改法_验收结果.md)确认 AC-01 至 AC-06 全部通过；最终全量回归于 2026-08-14T04:53:57+00:00 至 2026-08-14T04:54:29+00:00 执行 `scripts/test_all.sh`，退出码为 0，`425 passed in 32.27s`，代码快照为 `567f9583a50ce06a1d4db145d7dbbf8d2b8f84bc91d5402648609b8ce4c1a5a2`，机器记录为 REG-20260814T045357+0000-1fa4e994；整体验收已经确认，交付关系见[需求交付追踪表](../需求交付追踪表.md)。

本轮已把结构化诊断接口、底层事实透传、旧文字接口兼容、固定字段真实格式、校验凭据逐项差异、实施入场基线、完整实施观察范围和逐文件三方核对同步到本文。核心结果是：门禁一次列出当前能确认的全部真实问题，不能检查的项目明确写依赖，末尾只给一条有效下一命令；计划登记只决定允许修改什么，完整观察独立证明实际改了什么。产品设计与功能文档核对一致。
