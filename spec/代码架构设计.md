...
| 逐项机器记录与结果整理 | `src/workflow_loop/test_execution.py::run_prepared_tasks`；`src/workflow_loop/test_report.py::parse_test_report`（解析结构化测试报告函数）；`src/workflow_loop/acceptance_records.py` | 执行前删除旧报告，按主题和测试依赖运行后读取新报告，从具体用例重算计数，并要求实际目标集合与登记集合完全相等；程序根据当前机器记录生成或回填测试结果文档，混合主题汇总只统计自动化项且第 3 节人工项渲染为测试方式人工验收说明（不写自动化测试结果未执行），自动化项编号集合由 test_mapping.automated_test_ids_in_plan 单一数据源提供给生成器与校验器共用，纯人工主题只核对测试计划和人工交接状态 | 前置失败时停止依赖项；报告缺失、目标缺少或多出、零执行、有跳过、有失败、格式或计数冲突、适配器未知或退出码非零均清除旧成功；测试代码变化只清理直接受影响测试项；纯人工主题不伪造机器结果 | `tests/test_current_workflow_contracts.py::test_test_change_invalidates_only_affected_items`、`tests/test_current_workflow_contracts.py::test_structured_report_requires_exact_non_skipped_current_targets` |
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：本轮修复混合主题测试结果文档的两个缺陷并收敛生成端与校验端的自动化项取数口径：新增 test_mapping.automated_test_ids_in_plan 单一数据源；records.py 生成器汇总只统计自动化项、第 3 节人工项改渲染为测试方式人工验收说明；artifact_validation.py 校验器第 3 节检查分自动化小节与人工小节两组。；产品规则未变化（R12/R14 既有规定的实现背离被修正），无需返回 spec；代码已全部实现并通过 4 个主题测试、用户 Terminal 全量回归 632 passed 与最终回归门禁机器记录。；架构正文需更新 6.10 功能映射表中"逐项机器记录与结果整理"一行的事实描述（见架构正文变更表）。；当前有效机器记录：REG-20260910T104406+0000-e888b8e8、RUN-20260910T093527+0000-14382bc9、RUN-20260910T093533+0000-fd82fcac、RUN-20260910T093540+0000-9b8bc848、RUN-20260910T093905+0000-850ec22f
