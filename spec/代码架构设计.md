...
| 保存首次副本 | `src/workflow_loop/rollback.py::prepare_impl` | 保存计划文件原内容或“原本不存在”，同时把完整入场观察清单写入回退依据；准备前先确认入场后没有未授权变化——入场后已修改的文件可用 Git 自证放行：进场快照等于 HEAD 版本时用 HEAD 字节补首次副本，进场快照不存在的新文件直接放行，不能自证的按文件列出原因保持拒绝（不降低回退安全性） | 路径提前变化、未登记文件已经变化或副本不可信时拒绝实施，不把修改后内容冒充原内容 | `tests/test_rollback.py::test_prepare_rejects_unregistered_change_after_impl_entry` |
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：本轮架构变化：新增 code_symbols 模块（ast 符号索引与设计同步引用核对）；test_result 表三列机器回填（test_result_columns 与 _fill_machine_record_ids 扩展，回填先于空表守卫）；prepare_impl 首次准备 Git 自证出口（快照差异逐文件核对、新文件放行、不可自证按文件拒绝）；主题读取三处合并 topic_relations 表主题（中途加主题死循环修复）。；核对依据：三主题验收结果见 acceptance/ 目录三份文档（10 条 AC 全 passed，机器测试记录编号以当前状态有效集合为准）；最终全量回归记录编号 REG-20260908T124320+0000-b57493b8，退出码 0、612 项通过；整体验收已由用户确认。；当前有效机器记录：REG-20260908T124320+0000-b57493b8、RUN-20260908T124017+0000-a7774207、RUN-20260908T124017+0000-fd42cda4、RUN-20260908T124018+0000-40391717、RUN-20260908T124018+0000-4555d0e6、RUN-20260908T124019+0000-35295fd0、RUN-20260908T124020+0000-4b5babbc、RUN-20260908T124021+0000-331725e9、RUN-20260908T124022+0000-60f9b079、RUN-20260908T124032+0000-e4b32532
