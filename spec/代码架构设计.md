...
| 最近已完成的交付证据 | 当前轮次“计划路径非法时门禁一次说清而不是抛异常”的主题验收、最终全量回归和整体验收已通过；更早轮次的交付状态只作为历史证据保留 | [计划路径非法时门禁一次说清而不是抛异常_验收结果](../acceptance/计划路径非法时门禁一次说清而不是抛异常_验收结果.md)、[对应测试结果](../qa/计划路径非法时门禁一次说清而不是抛异常_测试结果.md)和本文第 9 节 |
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：主题验收事实见 acceptance/计划路径非法时门禁一次说清而不是抛异常_验收结果.md（AC-01 至 AC-04 结论均为 passed）；主题验收实际使用的机器测试记录编号：RUN-20260902T102333+0000-027544d0、RUN-20260902T102333+0000-082c055f、RUN-20260902T102334+0000-4e99bfff、RUN-20260902T102334+0000-8a2265d9、RUN-20260902T102334+0000-92e3d664、RUN-20260902T102334+0000-9bd570cc、RUN-20260902T102334+0000-ae6c8d90；最终全量回归执行事实：机器记录编号 REG-20260902T102411+0000-41526fea，命令 scripts/test_all.sh，退出码 0，539 项通过；整体验收确认：用户预授权由 AI 代行确认链，workflow gate overall_acceptance --confirmed 完成确认，全过程 journal 可复核；缺陷记录见 bug/缺陷_计划路径非法时门禁一次说清而不是抛异常.md。本轮只改计划路径失败的报告方式：解析处已构造的诊断随专用异常上抛，实施门禁报告同时收集计划与实际改动两侧诊断，命令层兜底按诊断渲染；判定口径和现有捕获方行为均未改变，改动落在既有模块内部，因此同步类型为架构未变化，对应事实写入本文第 6.3 节。
