...
本文记录 Workflow Loop 当前已经实现并完成验收的代码架构。维护者可以从每个产品功能直接找到真实代码入口、关键处理、状态或数据、失败结果和具体测试函数。
...
| 核对项 | 当前事实 | 依据 |
|---|---|---|
| 正式版本 | 永久固定为 0.1.0 | `pyproject.toml`、`src/workflow_loop/__init__.py`、安装脚本和发布流程 |
...
2. 安装器检查 Python 3.11 或更高版本、现有命令身份和完整写入范围；用户确认后，安装固定 0.1.0 全局命令与项目骨架。
...
| 产品规则 | 代码职责 | 实际承担位置 |
|---|---|---|
| 固定版本、跨平台、事务安装 | 终端适配与共同安装事务分离 | `install.sh`、`install.ps1`、`src/workflow_loop/installer.py` |
...
```mermaid
flowchart TB
    A["终端与发布适配层<br/>install.sh / install.ps1 / release.yml"]
    B["命令编排层<br/>cli.py"]
...
`install.sh` 负责 macOS 和 Linux 终端适配，`install.ps1` 负责 Windows PowerShell 5.1 与 PowerShell 7 适配。二者检查环境、打印范围、取得一次确认、下载并校验固定 uv 0.11.33，然后调用 Python 安装事务。`.github/workflows/release.yml` 固定版本身份、构建顺序和三平台安装验证。
...
产品要求见[功能_安装到项目.md](./功能_安装到项目.md)。安装必须使用现有 Python 3.11 或更高版本、固定 0.1.0、一次确认和完整事务；取消或失败不能留下半套项目。
...
| 骨架身份检查 | `src/workflow_loop/project.py::check_skeleton` | 核对固定版本、项目状态、两套资源仓库和智能体契约 | 残缺或版本异常时写入前停止 | `tests/test_project.py::test_is_installed_rejects_project_state_without_complete_skeleton` |
...
| 跨平台安装 | 固定 0.1.0，使用现有 Python，失败整体恢复 | Bash、PowerShell、Python 事务和三平台发布验证已经连通 | 一致 |
