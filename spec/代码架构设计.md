...
当前产品范围以[产品总说明](./产品总说明.md)链接的 18 份功能文档为准：
...
18. [工作记录表与正式文档生成](./功能_工作记录表与正式文档生成.md)
...
| 核对项 | 当前事实 | 依据 |
|---|---|---|
| 当前版本身份 | 源码版本从 `pyproject.toml`（Python 打包配置）读取；命令、六个维护脚本、项目安装标记和自动发布配置保持同一版本。产品文档引用[项目说明](../README.md)和[最新正式发布](https://github.com/yuzyf/workflow_loop/releases/latest)，不逐处写死产品版本号 | `scripts/release.py::update_release_identity`（更新当前发布身份函数）、`src/workflow_loop/__init__.py::__version__`（包版本号）和 `tests/test_version_document_sources.py::test_current_docs_use_shared_release_sources`（统一来源测试） |
...
| 最近保存的公开发布证据 | `0.2.0` 的标签、GitHub Actions 任务、PyPI 页面和 GitHub 正式发布页是已完成历史证据；当前版本身份是现行源码和发布配置事实，不用旧任务编号冒充新版本已发布证明 | [0.2.0 主题测试结果](../qa/0_2_0_在_GitHub_和_PyPI_完成正式发布_测试结果.md)、`tests/release_publication_checks.py`和当前版本身份文件 |
...
| 最近已完成的交付证据 | 本轮五个主题共十一条验收条件通过，最终全量回归 564 项通过，整体验收已按用户持续推进授权由 AI 代行确认；本轮没有提交、推送或发布 | [本轮主题验收索引](../acceptance/索引.md)、[本轮测试索引](../qa/索引.md)、[需求交付追踪表](../需求交付追踪表.md)和本文第 9 节当前机器证据 |
...
2. 安装器检查 Python 3.11 或更高版本、现有命令身份和完整写入范围；用户确认后，安装当前正式发布版本的全局命令与项目骨架；准确版本和命令由[项目说明](../README.md)统一提供。
...
公开使用入口是仓库根目录的中文 `README.md`（项目说明文档）。当前 README 展示四个公开状态徽章，说明产品定位、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台正式安装命令、最小使用示例、简洁命令表、使用 `uv`（Python 项目管理工具）的源码开发方式、仓库结构和详细设计入口。README 的安装入口与当前包版本保持一致，产品文档引用该入口而不复制具体版本；无需开发任务规则同步到安装器生成的 `AGENTS.md`（AI 契约文件）和 README。
...
| 产品规则 | 代码职责 | 实际承担位置 |
|---|---|---|
| 当前版本一致、跨平台、事务安装 | 终端适配与共同安装事务分离；所有当前安装身份与包版本一致，产品说明引用统一安装来源 | `install.sh`、`install.ps1`、`src/workflow_loop/installer.py` |
...
| 公开说明、包元数据和当前版本发布 | 版本常量、静态说明、许可证、打包配置和发布任务共同承载当前包版本；发布任务按与包版本一致的版本标签先验证、再发布 PyPI、最后创建 GitHub 正式发布页 | `src/workflow_loop/__init__.py`（产品版本常量）、`README.md`（项目说明文档）、`LICENSE`（许可证文件）、`pyproject.toml`（Python 打包配置）、`.github/workflows/release.yml`（自动发布配置） |
...
flowchart TB
    ...
    D["正式产物与追踪层<br/>artifact_paths.py / artifact_validation.py / records.py / design_sync.py / markdown_links.py / traceability.py"]
...
`README.md`（项目说明文档）和 `LICENSE`（MIT 许可证文件）属于同一公开说明与发布适配层，不被运行时命令调用。`pyproject.toml`（Python 打包配置）提供 README、作者 `yuzyf`、MIT 许可证、固定项目简介和仓库链接等分发元数据，并使说明与许可证进入构建产物。当前版本身份由包版本、项目说明和正式发布配置共同核对，本文不再复制固定版本号；自动发布层负责版本核对、全量测试、构建、三平台安装验证、PyPI 发布和 GitHub 正式发布页创建，历史发布证据留在对应测试与验收记录中。
...
`src/workflow_loop/records.py`（工作记录表与文档生成模块）按开工冻结版本提供栏目、校验事实并选择固定出口。它把生成路径、文档指纹和缺陷文档指纹的可信副本及正文摘要保存到每轮记录目录的 `.generated/`（程序生成凭据子目录），写入前先核对表内专用字段。生成器通过 `src/workflow_loop/markdown_links.py::with_heading_anchors`（补齐唯一标题定位的函数）与链接修复器共用导航规则。

缺陷表发现由 `records.py::bug_record_tables`（发现并核对本轮全部缺陷表函数）统一负责；每张表拥有独立生成凭据，旧单表路径继续保留。`_sync_bug_record_tables`（整批预检并生成缺陷文档函数）先检查全部输入与手改，再生成本次文件；共享索引按解析后的本主题行更新，其他主题事实保留。

`src/workflow_loop/design_sync.py::prepare_design_sync`（准备正式架构候选正文函数）接收最终同步表、可选正文变更表、现有架构和首次生成基准，先检查当前验收证据、唯一原文位置及正文冲突，再返回候选正文和生成凭据。`records.py::_sync_design_tables`（最终设计表同步函数）统一写入内部记录和正式架构；固定结论与全部当前机器编号从状态生成，不要求 AI 再手写一套字段。正文必须有真实基准，不从同步结论推测或编造架构内容。

<a id="46-实施测试与验收层"></a>
### 4.6 实施、测试与验收层

`src/workflow_loop/rollback.py` 保存首次原内容并负责恢复；`list_actual_working_tree_changes()`（读取 Git 实际工作树改动函数）和 `actual_implementation_paths_since_entry()`（取得进入实施后的实际路径函数）读取真实变化，`validate_actual_implementation_changes_report()`（按实际改动核对实施记录函数）只检查实际文件的逻辑、修改理由、验收关联和测试证据，不再比较计划文件集合。`src/workflow_loop/test_mapping.py` 通过 `all_test_source_paths()`（全部测试源码路径函数）和 `collect_all_workflow_test_markers()`（收集全部工作流测试标识函数）扫描全部测试源码，连接主题、测试项、验收条件和真实测试入口；`parse_official_target_names()`（解析正式目标名称函数）是登记入口形状的单一判断口径，工作记录表校验、按表登记和执行前登记校验都调用它，文档模式的 `_entry_parts()` 复用同一段形状判断。`src/workflow_loop/verification.py::list_actual_test_changes()`（读取实际测试改动函数）列出 Git 可见的测试代码、夹具、辅助脚本和配置变化。`src/workflow_loop/machine_collect.py`（机器事实采集模块）按实施前回退副本与进场 Git 基线提交号计算差异事实（回退副本优先、其次进场提交号、无登记按新文件全部新增），`collect_implementation_changes()`（采集实施改动函数）把文件与代码位置两列写入实施记录表并记录采集指纹，`verify_collection_fingerprint()`（核对采集指纹函数）供 impl 第二道门重算比对；位置值与门禁行号格式同源（新文件写 L1-L行数，多块写覆盖全部差异的连续范围）。`src/workflow_loop/code_symbols.py`（代码符号索引模块）用 ast 解析 src/ 与 tests/ 全部 Python 文件建立路径到符号集合索引（模块级函数、类、类方法，单文件语法错误跳过记录），`check_reference()`（引用核对函数）对 文件::符号 引用四分支核对（真实通过、错误给相近符号建议、非 Python 查存在、不存在文件拒绝）。`QaStage`（测试验证环节策略）统一用户可见环节，但保留各事实来源的独立边界：测试计划说明准备怎样验证，`src/workflow_loop/test_execution.py` 保存待执行任务和逐测试项机器记录，测试结果文档整理已经发生的结果。`src/workflow_loop/process_runner.py` 受控运行子进程。`src/workflow_loop/test_report.py`（结构化测试报告解析模块）适配项目已登记的测试框架，返回真实目标名称和执行统计；`src/workflow_loop/acceptance_records.py` 把当前机器记录或用户实际回答固化为验收记录。
...
- **对应产品内容**：公开产品身份、项目简介、作者、MIT 许可证和仓库链接保持一致；当前源码、README、安装脚本、分发元数据和发布任务统一使用当前包版本，已发布版本不覆盖或重发。
...
- **上游**：维护者完成版本号、说明、许可证、元数据和发布配置，把最终提交推送到主分支，再让 与包版本一致的正式版本标签指向该提交；GitHub Actions（GitHub 自动任务）读取标签并启动发布流程。
...
- **下游**：用户从[项目说明](../README.md)取得当前系统的准确安装命令；该命令指向对应版本的 GitHub 正式发布页，并从 PyPI（Python 包发布平台）安装同版本分发包。
- **状态和数据**：当前版本标签、项目包版本和正式发布名称均由当前发布身份文件给出；固定发布说明和六个维护脚本随该版本发布。本文不复制具体版本，历史版本的提交、任务编号和公开页面证据保持不变。
...
- **主要处理**：`validate_stage_output()`（执行阶段产物校验函数）在第二道门扫描受管链接并保存指纹；第三道门只复核绑定文件。`_DocumentCache`（单次调用的文档缓存）共享文件字节、读取错误和结构解析结果，同一目标的标题候选也复用；修复后的覆盖视图只使变化文档重新解析。`cmd_repair_links()`（链接修复命令函数）仅在预览分支规划，执行分支直接使用 `LinkRepairResult.unresolved`（已经复查的剩余问题），不额外重做全量规划。
- **状态和数据**：修复前独立核对来源与目标的真实磁盘字节、符号链接和预览指纹，再用内存中的待写内容复查结果；事务清单与原文副本保存成功后才逐文件原子替换。写后继续独立读取真实磁盘，确认来源未变、目标与待写内容一致；缓存不代替这些磁盘检查。失败恢复全部修复目标，非目标文件的并发编辑保留。
- **失败结果**：来源或目标哈希变化时零写入；候选不唯一、目标标题已删除或位于非受管文档时只报告。写入失败或复扫发现候选问题未消失、新增问题时恢复全部原文并核对哈希；恢复也失败时保留事务副本并报告部分现状，不声称成功或零修改。
- **验证位置**：`tests/test_workflow_guidance.py::test_link_scans_reuse_unchanged_documents`（重复读取解析计数测试）、`tests/test_workflow_guidance.py::test_regeneration_preserves_anchors_and_manual_edits`（定位再生及手改保护测试）和`tests/test_workflow_guidance.py::test_link_repair_keeps_transaction_guards`（漂移及事务恢复测试）覆盖本轮变化；`tests/test_markdown_links.py`（既有链接测试）与 `tests/test_commands.py`（命令测试）继续覆盖解析、边界和中断恢复。
...
产品要求见[功能_安装到项目.md](./功能_安装到项目.md)。当前安装使用现有 Python 3.11 或更高版本、项目说明中指向的正式目标版本、一次确认和完整事务；取消或失败不能留下半套项目。用户先从公开 README 了解产品和环境要求，再使用当前系统的正式安装入口。
...
| 骨架身份检查 | `src/workflow_loop/project.py::check_skeleton` | 核对产品版本、项目状态、两套资源仓库和智能体契约；目标版本必须与正在使用的包版本一致 | 残缺或版本异常时写入前停止 | `tests/test_project.py::test_is_installed_rejects_project_state_without_complete_skeleton` |
...
| 公开说明入口 | `src/workflow_loop/__init__.py::PRODUCT_IDENTITY`（命令版本身份）；`README.md`（根目录项目说明文档） | README 展示四个徽章、统一产品身份、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台命令、最小示例、简洁命令表、`uv`（Python 项目管理工具）源码开发方式、仓库结构和详细文档链接；版本身份与项目说明中准确安装命令指向的版本保持一致 | 文件缺失、结构不完整、出现旧仓库当前链接或命令版本不一致时，公开项目测试失败 | `tests/test_public_project.py::test_readme_has_the_confirmed_chinese_structure_and_entries`、`tests/test_light_task.py::test_installed_contract_and_readme_describe_the_light_task_rules` |
...
继续轮次时，`src/workflow_loop/cli.py::restore_recovery_context_from_journal`（从日志补回恢复说明函数）与材料加载检查共用 `src/workflow_loop/journal.py::belongs_to_workflow`（判定日志所属轮次函数）。显式编号必须精确匹配当前轮次；无编号旧日志只有时间可解析、带时区且不早于本轮开工才参与恢复。`src/workflow_loop/topic.py::current_workflow_topics`（当前主题读取函数）把 state.topics 与 topic_relations 工作记录表主题合并去重，验收计划确认门与表同步同样合并——中途新增主题不再被索引待生成占位阻断（索引需要文档、文档需要主题解析的死循环修复）。已经处理的恢复标记也按同一轮次边界过滤，因此不会拿七月的失效事件解释九月首次进入阶段。`tests/test_recovery_journal_boundary.py::test_foreign_and_invalid_entries_do_not_restore`（旧轮次和非法时间拒绝测试）与 `test_current_legacy_entries_and_handled_markers`（同轮兼容及已处理标记测试）覆盖两类分支。
...
| 代码实施第二道门按实际改动检查 | `src/workflow_loop/cli.py::cmd_gate`、`src/workflow_loop/rollback.py::validate_actual_implementation_changes_report`（按实际改动核对实施记录函数） | `impl` 第二道门列出本次实际文件、是否属于本轮、修改理由、验收关联、测试证据和缺失项；文件未出现在最初计划中不算错误；默认下一步不要求 `--prepare-code`（准备代码回退基线）或 `--rebaseline`（重设基线）。机器采集由 `src/workflow_loop/cli.py::cmd_collect`（机器采集命令函数）和 `src/workflow_loop/machine_collect.py::collect_implementation_changes`（采集实施改动函数）承担：`workflow collect` 命令只适用于 impl 环节，按差异事实写实施记录表的文件与代码位置两列并记录采集指纹（AI 叙述列保留；进场代码干净时由 `src/workflow_loop/rollback.py::prepare_impl` 调用 `record_entry_git_baseline`（记录进场基线提交号函数）记 Git 提交号供计划外文件取进场内容），第二道门对表内已有指纹的主题重算差异并比对，不一致报请重新采集 | 缺理由、缺验收关联或缺测试证据时一次列全失败项；恢复依据不足单独显示限制 | `tests/test_current_workflow_contracts.py::test_actual_change_gate_reports_each_missing_evidence`；`tests/test_current_workflow_contracts.py::test_unplanned_implementation_file_passes_with_complete_evidence` |
...
| 最终设计同步 | `src/workflow_loop/records.py::_sync_design_tables`（按表同步设计函数）、`src/workflow_loop/design_sync.py::prepare_design_sync`（准备正式架构候选正文函数）和 `src/workflow_loop/stages/stages.py::UpdateCodeDesignStage`（最终同步阶段） | 同步表提供六项结论，正文变更表按章节与原文唯一定位；从首次基准精确替换指定事实，保留其余正文、标题、表格和锚点；当前轮次和完整机器编号由有效状态生成，写前通过真实映射校验 | 产品文档变化要求返回产品设计；缺当前证据、原文重复或手改冲突时保留正式架构；缺少真实正文基准时拒绝编造 | `tests/test_design_sync_tables.py::test_stage_generates_current_formal_architecture`（正式出口与机器证据测试）、`tests/test_design_sync_tables.py::test_body_changes_preserve_structure_and_repeat`（正文稳定测试）、`tests/test_design_sync_tables.py::test_invalid_evidence_and_conflicts_never_overwrite`（缺证据和冲突零覆盖测试） |
...
| 真实映射校验 | `src/workflow_loop/artifact_validation.py::validate_final_code_design_document` | 逐个当前功能检查真实文件、可定位符号与验证位置；最终核对依据中的机器编号集合必须精确等于当前有效主题记录和最终回归记录集合。 | 缺少、额外、拼接错误或失效编号均拒绝；提示测试和回归重跑会使旧编号失效，要求重新取得当前有效集合，不再把全部差异归为拼写错误。 | `tests/test_workflow_guidance.py::test_final_sync_explains_obsolete_record_ids`（当前机器编号精确集合测试） |
...
产品要求见[功能_复现产品缺陷.md](./功能_复现产品缺陷.md)。一轮修复可以处理多个缺陷；每个缺陷独立记录真实输入、实际结果、期望结果、根因、证据和一个唯一验收主题。程序整批发现本轮记录、核对身份后分别生成，不用一张表或一份凭据代表全部缺陷。

| 场景 | 代码位置 | 关键逻辑与状态 | 异常结果 | 验证位置 |
|---|---|---|---|---|
| 复现阶段门禁 | `src/workflow_loop/stages/stages.py::ReproduceStage`（缺陷复现阶段）、`src/workflow_loop/records.py::bug_record_tables`（发现并核对本轮全部缺陷表函数） | 读取本轮全部缺陷表、正式记录和共享索引；一份缺陷对应一个唯一主题，同轮允许多个不同主题，旧基础表保留原路径 | 复现事实不全、重复主题、缺表、轮次或文件标识冲突均拒绝；任一输入失败时不覆盖本批正式文档 | `tests/test_multibug_workflow.py::test_independent_bug_documents_and_receipts`（多份记录独立生成测试）、`tests/test_multibug_workflow.py::test_legacy_and_invalid_bug_tables_preserve_facts`（旧表与非法输入测试） |
| 复现文档校验 | `src/workflow_loop/artifact_validation.py::validate_reproduce_documents`（复现文档校验函数）与 `src/workflow_loop/records.py::validate_table`（工作记录表校验函数） | 每个主题的记录表分别检查并对应本主题正式文档。新轮次使用版本 3：根因说明、根因位置、根因证据、运行环境和真实输入分栏填写，按表校验后生成，不再从正文拆标签。版本 1 和 2 保留原栏目及旧文档检查；工作流编号、主题、索引和章节仍核对。 | 新版缺少任一必需事实时指出具体栏目；根因中的换行或标签引用不被截断。确无技术不确定性只允许整栏一条“暂无”，其他必需事实中的占位内容仍拒绝。 | `tests/test_workflow_guidance.py::test_bug_facts_are_separate_and_literal`（独立事实及原文保留测试）；`tests/test_workflow_guidance.py::test_no_uncertainty_is_explicit_and_strict`（暂无与必需事实边界测试） |
...
| 缺陷记录锚点与手改保护 | `src/workflow_loop/records.py::_sync_bug_record_tables`（整批预检并生成缺陷文档函数）、`src/workflow_loop/records.py::_existing_bug_result_blocks`（读取既有阶段结论块函数） | 每个缺陷有独立内部记录、正式文档和生成凭据；写前核对程序字段及本主题正文摘要，保留合法定位和第八节后续验收事实；旧基础表继续使用原凭据路径 | 第二个缺陷不再串用第一个凭据；篡改或手改冲突时拒绝本批生成，未改主题事实不被覆盖 | `tests/test_multibug_workflow.py::test_independent_bug_documents_and_receipts`（凭据隔离和重复生成测试）、`tests/test_workflow_guidance.py::test_program_fields_reject_tampering_before_writes`（程序字段防篡改测试） |
| 缺陷索引按内容判定 | `src/workflow_loop/artifact_validation.py::validate_reproduce_documents`（复现文档校验函数）、`src/workflow_loop/bug_record.py::index_entry`（解析本主题索引行函数）与 `src/workflow_loop/bug_record.py::replace_index_entry`（替换指定索引行函数） | 共享索引兼容两列与四列结构，只替换指定缺陷行，保留其他主题及历史行；重复入口拒绝。校验只检查索引存在且链接本轮缺陷记录，不再要求索引出现在本阶段变化清单；变化清单里没有缺陷记录时按文件内的工作流编号识别本轮记录，不依赖字节变化 | 索引内容已经正确时不报“没有在本阶段更新”；表模式下重走本环节、文档与上次逐字节相同时仍能找到本轮缺陷记录；索引确实缺链接时仍报对应失败 | `tests/test_gate_rework_stability.py::test_bug_index_is_judged_by_content_not_by_change` |
| 主题登记 | `src/workflow_loop/topic.py::list_reproduce_topics`（读取本轮缺陷主题函数）和 `src/workflow_loop/records.py::ensure_stage_tables`（准备当前阶段表函数） | 从当前轮次全部缺陷记录读取主题；首次验收计划索引尚不存在时按已确认主题建表，再由表生成索引，不形成互相等待 | 同轮多个不同主题允许继续；每份缺陷的主题缺失、重复或输入错误由复现校验报告；首次索引缺失不阻止建表，已存在索引的格式错误仍报告 | `tests/test_multibug_workflow.py::test_legacy_and_invalid_bug_tables_preserve_facts`（首次建表、旧表兼容与错误边界测试） |
...
| 设计基线与跳过复用分流 | `src/workflow_loop/cli.py::ensure_spike_baseline`、`src/workflow_loop/cli.py::cmd_gate`（含 `--skip`/`--reuse` 分支）与 `src/workflow_loop/state.py::WorkflowState`（含 `spike_reused` 复用状态字段） | 进入穿刺前保存产品和现状架构哈希，供结论判断是否真实改变后续决定；代码计划随后读取结论，不要求穿刺阶段直接改未来计划。跳过与复用分开：`--skip` 检测到本工作流已登记穿刺资产时先打印警告（数量、验收计划将无法引用、指路 `--reuse`）并退出，用户知情后再次执行才跳过；`--reuse` 校验复用依据（非占位词且不少于 12 字符）后置复用标记并记录依据，复用跳过的推进路径；`workflow discuss` 的 spike 材料清单输出返回重走决策树三条路径（重新穿刺 / skip / reuse）并列示已登记资产 | 需要证明变化却没有可靠基线时不放行；无资产时 `--reuse` 拒绝；无依据或占位词依据的 `--reuse` 按内容问题拒绝 | `tests/test_spike_validation.py::test_validate_spike_stage_requires_design_hash_change`、`tests/test_gate_spike_reuse.py::test_reused_state_releases_traceability_check`、`tests/test_gate_spike_reuse.py::test_discuss_prints_decision_tree` |
...
| 发现实际实施变化 | `src/workflow_loop/rollback.py::actual_implementation_paths_since_entry`（取得进入实施后的实际路径函数）；`src/workflow_loop/verification.py::compute_complete_implementation_file_snapshot`（完整实施观察快照函数） | 通过观察快照和 Git 可读差异取得代码、脚本、配置及包内流程材料的实际变化；Git 工作区清单按白名单过滤且与观察快照同源（`src/workflow_loop/verification.py::is_implementation_related_path`，支持项目级扩展后缀——`src/workflow_loop/project.py::ProjectState` 的 `extra_code_suffixes` 字段经 project.json 登记），非代码临时文件（原型图等）不算实际改动，疑似临时产物在输出中附存放约定提示（指向轮次草稿目录）不阻塞；计划确认前不把变化自动改写成计划错误 | 列出新增、修改或删除的具体路径；恢复范围不足单独显示限制，不阻塞实际改动证据门禁 | `tests/test_current_workflow_contracts.py::test_unavailable_change_or_recovery_scope_is_non_blocking` |
...
| 保存首次副本 | `src/workflow_loop/rollback.py::prepare_impl` | 保存计划文件原内容或“原本不存在”，同时把完整入场观察清单写入回退依据；准备前先确认入场后没有未授权变化——入场后已修改的文件可用 Git 自证放行：进场快照等于 HEAD 版本时用 HEAD 字节补首次副本，进场快照不存在的新文件直接放行，不能自证的按文件列出原因保持拒绝（不降低回退安全性）；被删除的文件同样放行——进场快照记录存在且 HEAD 有该路径时用进场快照哈希做 Git 自证补 HEAD 字节副本（哈希对比不依赖工作区状态），HEAD 也没有该路径（从未提交过）时回退清单登记无副本及原因放行，不要求恢复文件 | 路径提前变化、未登记文件已经变化或副本不可信时拒绝实施，不把修改后内容冒充原内容 | `tests/test_rollback.py::test_prepare_rejects_unregistered_change_after_impl_entry` |
...
| 登记执行任务 | `src/workflow_loop/cli.py::_test_registration_instruction`（按当前模式生成登记指引函数）、`src/workflow_loop/cli.py::_prepare_tasks_from_tables`（从表批量登记函数）、`src/workflow_loop/test_execution.py::prepare_task`（登记任务函数） | 表模式直接提示 `workflow test prepare --from-tables`（从表登记命令）；旧文档模式列出主题、测试项、报告适配器及真实命令。任务保存参数数组、目录、入口、依赖、超时和报告设置；批量输入不变时保留已有任务及成功记录，变化时只重建对应项；逐条登记仍默认重建以支持主动重跑 | 路径越界、命令或正式目标不合法、报告设置冲突时当场拒绝；纯人工项不登记，旧文档模式加载材料不自动建空表或迁移 | `tests/test_registration_guidance.py::test_table_next_step_registers_every_task_first_time`（首次登记及重复登记保留测试）、`tests/test_registration_guidance.py::test_legacy_registration_keeps_required_parameters`（旧模式完整参数测试）、`tests/test_table_entry_registration.py::test_two_entries_in_one_cell_register_and_pass`（多入口登记测试） |
...
| 场景 | 代码位置 | 关键逻辑与状态 | 异常结果 | 验证位置 |
|---|---|---|---|---|
| 主题验收记录 | `src/workflow_loop/acceptance_records.py::ensure_automated_records`（从当前测试建立自动验收记录函数）、`src/workflow_loop/records.py::_fill_acceptance_record_ids`（从有效状态回填验收编号函数） | 自动化条件绑定当前机器记录；生成验收结果时按轮次、主题和验收条件取得有效验收编号，核对方式、结论与机器依据，不信任手填编号；人工部分仍保存用户原话和证据，冻结版本一保持原结构 | 缺失、失效、串用记录或机器依据不一致时拒绝生成，保留原正式文档；退回一个主题只使该主题结果失效 | `tests/test_multibug_workflow.py::test_return_only_invalidates_selected_bug`（真实验收编号生成、错误拒绝和定向退回测试）、`tests/test_acceptance_records.py::test_automated_acceptance_record_is_created_from_current_test`（自动验收记录测试） |
| 最终全量回归 | `src/workflow_loop/test_runner.py::run_final_regression` | 选择当前系统入口，在项目根执行一次并保存参数、时间、退出码、输出和代码哈希。执行前先由 `src/workflow_loop/test_runner.py::should_skip_final_regression`（判定是否跳过最终回归函数）做跳过判定：按实施前回退清单计算差异并排除受管文档，产品代码与测试代码差异均为空才跳过（依据与时间写入 state 的 skip_reason/skipped_at，状态 skipped-by-no-change，不产生机器执行记录；设计同步不要求 REG 编号）；仅测试代码变化不跳过；清单缺失或差异计算失败一律运行。测试结果表三列（测试项编号、执行结论、机器记录编号）由 `src/workflow_loop/records.py::_fill_machine_record_ids`（机器记录回填函数）在表同步时先于空表守卫自动回填：已有行按编号对齐更新、无行按当前任务追加，AI 只填实际结果说明叙述列 | 非零退出、超时、入口或代码变化时不进入整体验收 | `tests/test_test_runner.py::test_final_regression_runs_registered_entry_and_records_machine_facts` |
...
| 场景 | 代码位置 | 关键逻辑与状态 | 异常结果 | 验证位置 |
|---|---|---|---|---|
| 完整预检 | `src/workflow_loop/rollback.py::preflight_abort` | 合并开工与实施清单，检查副本、哈希、路径安全和恢复目标；作废清单逐文件带出 `no_copy_reason`（删除时无 Git 提交记录等原因）并汇总 `no_copy_paths`，预检输出列出无副本文件清单并说明实施前内容无法取得、保持当前删除状态 | 任一问题都保持整轮进行中且不开始恢复；无副本文件不阻塞作废、不伪造恢复 | `tests/test_rollback.py::test_tampered_backup_blocks_restore_before_project_change`、`tests/test_rollback.py::test_full_abort_restores_managed_content_and_project_fields_only` |
...
| 场景 | 代码位置 | 关键逻辑与状态 | 异常结果 | 验证位置 |
|---|---|---|---|---|
| 生成空表 | `src/workflow_loop/records.py::create_or_complete_table`（创建或补齐工作记录表函数） | 按本轮冻结版本和主题建表并建立独立生成凭据；缺陷表支持同轮多份，旧基础表不改路径；首次验收计划建表不依赖尚未生成的本轮索引。旧表重新加载时更新过时填写说明，不改已填事实。版本 1 和 2 保持原结构，新轮次才使用版本 3 的独立缺陷事实栏。 | 表文件损坏或路径不安全时报结构化错误，不裸崩 | `tests/test_workflow_guidance.py::test_hints_refresh_without_invalidating_facts`（刷新说明保留事实测试）；`tests/test_workflow_guidance.py::test_legacy_tables_keep_frozen_fields`（旧表冻结结构及接续测试） |
| 校验表内容 | `src/workflow_loop/records.py::validate_table`（工作记录表校验函数） | 按冻结版本核对栏目、类型、编号和必需事实，先验证程序专用字段与独立生成凭据一致；只有声明允许表达确无内容的叙述栏才接受整栏一条“暂无”，空栏和混合占位内容仍拒绝。 | 栏目名写错、多写、少写、必填空或编号重复时一次拒绝 | `tests/test_workflow_guidance.py::test_bug_facts_are_separate_and_literal`（新版必需事实测试）；`tests/test_workflow_guidance.py::test_no_uncertainty_is_explicit_and_strict`（确无内容边界测试） |
| 生成正式文档 | `src/workflow_loop/records.py::generate_document`（按表生成正文函数）、`src/workflow_loop/records.py::generate_test_result_document`（按机器记录生成测试结果函数）及 `src/workflow_loop/markdown_links.py::with_heading_anchors`（生成唯一标题定位函数） | 正式文档按表确定性生成；测试编号和验收编号分别从对应有效记录回填。最终同步由 `src/workflow_loop/records.py::_sync_design_tables`（设计表同步函数）和 `src/workflow_loop/design_sync.py::prepare_design_sync`（正式架构候选生成函数）连接正式架构出口，先验证完整机器依据及正文冲突；固定章节和条目共用唯一标题定位，合法历史大小写别名继续保留。产品功能小节、缺陷正式出口和上游引用回补也使用同一保护规则。 | 表文件无法解析时报结构化错误，不裸崩 | `tests/test_workflow_guidance.py::test_regeneration_preserves_anchors_and_manual_edits`（再生与回补保留定位测试） |
| 检测手改 | `src/workflow_loop/records.py::sync_stage_tables`（阶段表同步函数）、`src/workflow_loop/records.py::_program_field_problems`（核对程序专用字段函数）和 `src/workflow_loop/records.py::_document_was_edited`（检测正文手改函数） | 生成路径由表类型和稳定标识计算；写前核对三个程序字段与独立凭据，再比较真实正文与可信正文摘要。只排除合法且紧邻唯一标题的标准定位，未知定位、重复定位和代码示例继续参与正文比较。 | 专用字段篡改时拒绝且不写正式文档；真实正文手改保持原文并报告。旧表没有凭据时只接纳能够按真实生成内容核实的旧值，不采用任意磁盘正文作为新基线。 | `tests/test_workflow_guidance.py::test_program_fields_reject_tampering_before_writes`（十类表写前拒绝测试）；`tests/test_workflow_guidance.py::test_regeneration_preserves_anchors_and_manual_edits`（正文保护测试） |
| 空表兼容 | `src/workflow_loop/records.py::workflow_uses_tables`（判断开工是否启用表流程的函数）和 `src/workflow_loop/records.py::table_is_filled`（检查是否已填事实的函数） | 是否启用表流程由开工冻结信息决定；已启用的轮次即使表为空也保持表流程并指出尚未填写，不能静默退回文档模式。测试结果行清单 required_at_gate 为 True，空 test_result 表在 qa 第二道门立即报尚未填写（2026-09-08 实证修复）。新轮次生成版本 4 表：实施记录表放行采集指纹程序专用键（AI 不填写）；验收结果表五列（验收方式、验收结论、机器测试记录编号、用户实际回答、人工确认）由 `_fill_acceptance_record_ids`（从有效状态回填验收编号函数）版本 4 分支从当前有效验收记录直接回填，失效条目标记待重做；版本 1 至 3 按开工冻结版本原样校验不接受新键。没有启用工作记录表的历史轮次沿用旧文档检查。 | 表缺失、损坏或未填时停留当前环节并报告；已填事实与旧轮次冻结版本不被重建或迁移。 | `tests/test_records.py::test_pristine_table_activates_table_path_and_reports_unfilled`；`tests/test_empty_result_table_guard.py::test_empty_test_result_table_reports_unfilled`（空表守卫测试）；`tests/test_machine_collect.py::test_v4_impl_record_accepts_collection_fingerprint`、`tests/test_machine_collect.py::test_fill_acceptance_row_columns_automated`（版本 4 与五列回填测试） |
...
| 按表登记测试 | `src/workflow_loop/cli.py::_prepare_tasks_from_tables`、`src/workflow_loop/test_execution.py::prepare_task` | 测试项填表后使用 `workflow test prepare --from-tables`（从工作记录表登记测试的命令）一次登记全部任务；输入未变化的任务与成功记录保留，输入变化时只重建对应项；命令层把项目目录传给表校验，合法程序回填仍能通过独立凭据核对，机器记录编号由程序维护。 | 表中测试项格式错或缺命令时按表检查失败 | `tests/test_records.py::test_table_create_then_recreate_keeps_filled_content` |
...
| 失效绑定盯表 | `src/workflow_loop/verification.py::_table_document_hash`（主题表业务摘要函数）和 `src/workflow_loop/verification.py::_stage_records_content_hash`（阶段表业务摘要函数） | 两种摘要都只计算填写事实，排除生成路径、文档指纹、缺陷文档指纹及填写说明；程序更新说明或回填生成值不使下游失效，真实事实改动仍改变摘要。 | 仅重新生成文档不使下游失效；表内容变化才失效 | `tests/test_workflow_guidance.py::test_hints_refresh_without_invalidating_facts`（程序字段与事实摘要边界测试） |
...
| 回补引用不覆盖机器事实 | `src/workflow_loop/records.py::_refresh_stage_document` 与 `_backfill_referencing_documents` | 新下游文档生成后只刷新实际引用方；回补前先验证程序字段与正文凭据，拿到当前工作流及测试任务后才生成测试结果。首次接续旧表先核实上游旧凭据，再加入新引用，避免程序自己的回补被误判为手改。 | 拿不到机器任务集时返回空并保持文档逐字节不变，不用空任务集把已通过结果刷成未执行 | `tests/test_workflow_guidance.py::test_regeneration_preserves_anchors_and_manual_edits`（再生及回补保护测试）；`tests/test_return_rework_flow.py::test_reference_backfill_keeps_machine_facts`（回补保留机器事实测试） |
...
`src/workflow_loop/markdown_links.py::without_generated_anchors`（计算正文时排除合法标准定位的函数）与 `with_heading_anchors`（生成唯一标题定位的函数）共享同一识别规则。只有实际标记、紧邻唯一匹配标题且自身不重复的标准定位可以排除；代码块里的示例、未知定位和重复定位都保留在正文摘要中。生成器与修复器因而不会互相抹掉结果，也不会借保留定位放过正文手改。
...
程序生成凭据验证文档写入来源，业务摘要验证上游事实变化，两者不能互相代替。`src/workflow_loop/records.py::_save_generation_receipt`（保存生成凭据的函数）记录程序字段和正文摘要；`src/workflow_loop/verification.py::_table_document_hash`（主题业务摘要函数）与 `_stage_records_content_hash`（阶段业务摘要函数）排除全部三个程序字段及填写说明，仅让真实事实变化触发下游失效。
...
发布和更新共同依赖产品版本、PyPI 正式版本和项目 `project.json` 安装版本字段。`scripts/release.py::read_current_version`（读取当前发布版本函数）只从 Python 打包配置取得旧版本，`update_release_identity`（更新当前发布身份函数）只写固定当前身份文件和项目安装版本字段；当前版本测试从同一个包版本派生预期，历史发布测试保持固定事实。`src/workflow_loop/cli.py::_resolve_update_version` 读取更新目标并核对 GitHub Release（GitHub 正式发布）；`src/workflow_loop/project.py::inspect_skeleton_for_update` 判断旧项目是否完整、版本是否允许升级；`src/workflow_loop/installer.py::update_project` 只写版本字段和静态管理文件。版本判断不复用 `check_skeleton()` 对“必须等于当前包版本”的安装判断，否则旧项目会先被误报成残缺，无法进入更新。 当前产品和安装文档统一引用[项目说明](../README.md)和[最新正式发布](https://github.com/yuzyf/workflow_loop/releases/latest)，不再逐处写死产品版本；历史发布说明和证据保留原版本。`tests/test_version_document_sources.py::test_current_docs_use_shared_release_sources`（版本更新与历史保留测试）在隔离副本中实际更新版本身份，确认当前引用有效且历史文字不被改写。
...
本轮五个主题、十一条验收条件内暂无未处理差异。[缺陷登记](../docs/待修复缺陷登记.md)中的第 13 号沿用既有修复并通过全量回归；第 14 至 22 号已完成本轮修复和验证。多缺陷不再共享单张凭据，恢复日志按开工边界隔离，表模式与旧文档模式各自给出可执行指引，重复登记保留未变成功记录，验收编号从有效状态回填，最终同步由产品生成器直接更新正式架构。本文内容来自同步表与正文变更表，不再使用一次性转换脚本代替产品生成器。
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：四个主题的产品规则、功能文档、代码实现和功能到代码映射逐项核对一致：实施并保护项目修改 R34-R36 映射 verification.py 白名单判定、rollback.py Git 清单过滤与删除放行、project.py 扩展后缀；整轮作废并恢复 R22 映射 rollback.py 无副本清单与 cli.py 作废输出；推进并确认 R47 加表 R30 映射 records.py 声明行与渲染、artifact_validation.py 声明放行、stages.py spec 门分支；验证技术不确定性 R30-R32 映射 cli.py skip 警告与 reuse、state.py spike_reused、traceability.py 连坐拆除与决策树。九个源文件实现与验收条件一一对应，17 项主题测试与 628 项全量回归全部通过。；本轮引入三处架构层面新机制（项目级扩展后缀配置、spike 复用状态、主题级表声明行），属架构变化，正文变更四行已把真实结构写回代码架构设计第 6.7、6.9、6.13 节。；当前有效机器记录：REG-20260910T060359+0000-9efe59f5、RUN-20260910T032610+0000-47d0e99f、RUN-20260910T032610+0000-4f3cfae1、RUN-20260910T032628+0000-03882904、RUN-20260910T032629+0000-2ca0aeb4、RUN-20260910T032646+0000-7c3697d4、RUN-20260910T032647+0000-21a2671f、RUN-20260910T032703+0000-01bb1989、RUN-20260910T032708+0000-5fc85b04、RUN-20260910T032720+0000-8eccf5c9、RUN-20260910T032726+0000-e590f518、RUN-20260910T032737+0000-0197415d、RUN-20260910T032742+0000-62df1ed3、RUN-20260910T032755+0000-38fedfce、RUN-20260910T032801+0000-98c749c8、RUN-20260910T032813+0000-c00048e6、RUN-20260910T032818+0000-78b95956、RUN-20260910T032835+0000-c338e7d4

<!-- workflow-loop:design-sync-checks:start -->
| 核对项 | 核对结论 | 设计影响 | 代码影响 |
|---|---|---|---|
| 产品设计核对 | 一致 | 无需修改 | 无需修改 |
| 功能文档核对 | 一致 | 无需修改 | 无需修改 |
...
<!-- workflow-loop:design-sync-checks:end -->
