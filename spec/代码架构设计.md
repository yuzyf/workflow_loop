...
| 最近已完成的交付证据 | 当前轮次“代码门禁按实际改动验证并展示过程”的主题验收、最终全量回归和整体验收已通过；更早轮次的交付状态只作为历史证据保留 | [代码门禁按实际改动验证并展示过程_验收结果](../acceptance/代码门禁按实际改动验证并展示过程_验收结果.md)、[对应测试结果](../qa/代码门禁按实际改动验证并展示过程_测试结果.md)和本文第 9 节 |
...
| 空表兼容 | `src/workflow_loop/records.py::table_is_filled` | 空表（未填任何内容）不启用表路径，旧轮次走原有检查 | 未填表的环节和旧轮次行为与改造前完全一致 | `tests/test_records.py::test_pristine_table_does_not_activate_table_path` |
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：本轮全部 475 项测试通过（含新增 3 项），七个验收主题 23 条验收条件全部通过；主题测试机器记录 RUN-20260829T215349+0000-5f61a784, RUN-20260829T215349+0000-d70ce790, RUN-20260829T215349+0000-e2c9d8ac, RUN-20260829T215349+0000-e8304148, RUN-20260829T215350+0000-4d8ad92b, RUN-20260829T215350+0000-53b3dd0a, RUN-20260829T215350+0000-623b916b, RUN-20260829T215350+0000-700956b8, RUN-20260829T215351+0000-1af2c3d6, RUN-20260829T215351+0000-946816d5, RUN-20260829T215351+0000-a08a100f, RUN-20260829T215351+0000-ccf906eb, RUN-20260829T215351+0000-d89ad32d, RUN-20260829T215351+0000-f6ca7679, RUN-20260829T215352+0000-2a0f873b, RUN-20260829T215352+0000-45a53c6a, RUN-20260829T215352+0000-90302500, RUN-20260829T215352+0000-bb7eb0a6, RUN-20260829T215352+0000-bbfaa82d, RUN-20260829T215352+0000-bdc1bade, RUN-20260829T215352+0000-d3c8c708, RUN-20260829T215353+0000-6948a287, RUN-20260829T215353+0000-fff073d4；最终全量回归 `scripts/test_all.sh` 退出码 0，机器记录 REG-20260829T220408+0000-12e67f57；验收记录编号 0216bebbbef9dd3b5a81007832f529a6fa75e644b5c56b0ebebd89ce576bdb2d, 1464a37af8a1a5a2547b6af47e5e48fb29a69bfed0bfb354484170f47f4b5330, 22d403c0cf7f6f4e14fc25fdb371e9610bb61be3a9834be82d25d15e409705b1, 32cc9ecfa98001817d3bef121a6f6b59b14d41cb3e319d1dcab175e0e146f635, 350a91d2b5aa5db719fc5cf807ea8c9e36e17a5e0fc81e2c771484d0d9b7e98e, 39b46ef0d0c84ebf92f56bb2c5dc949daa6f22a9d0b2807ea7a16c50b0877dca, 3ad50236c18f6348dd01b034cd2e79885b5d0531b4de02f5d30830004dbabfff, 4855a7d5b4b5f21c56c9144446f7c6a760143133c55728ef99d9a238e60223c6, 54389937830ad330506fdbd34dcb12c013f6a2491a4944dbdc6b5f34359049ad, 66434a5c981018215cdbadc578203d9a94088019b8744ed146eea17dbd5602d7, 6718ff0485ddaf851663ad368094f5213eab7fc02013db0ac9df5316c5d3feb1, 6e71363f079eb3b1d1ea5c09fbf3f41fa1ed07fc1709d85dd9b61b9477a9a8d0, 9222186567723a406d6a4855c5995f45fcf55d8807201231e56283c6dda7f3c9, 9451de51935bd8c2840e51386ab4e6cef2ee7982e3134b1b2b218b6bdedd6e45, 9fd906ff7c0b7245bad848c4b8a8fadb2d867f289413e9617756e8be6def5ec4, c50dafb0bc4f3de4f8d4a844c431a751196c6bc508e7c681276968c9bac5c0af, c9395c90e8102285e80d4865fd3ea800bcc94fa387d2143a59039c3c449f13fa, cfc3cd71eade40dfdc29fcb872908bb565e6ae9b33344744fb0e8691e2b82454, ff376c45e0ca0807aeb0006065ba3a3a0085d0b86322777d1fb6bb3bf337c1be；交付关系见[需求交付追踪表](../需求交付追踪表.md)。

本轮已把实际改动门禁、首次材料编号五步过程、测试改动定向失效和恢复依据与门禁分离同步到本文。核心结果是：计划只说明预期，实施和测试门禁读取 Git 与入场观察得到的实际文件，缺理由、验收关联或测试证据才失败；回退清单缺失只显示恢复限制，不阻塞代码或测试门禁。产品设计与功能文档核对一致。

---

## 最终设计同步（2026-08-29-0852-product_change）

### 新增模块

- `src/workflow_loop/records.py`（约 700 行）：工作记录表的核心模块。定义 10 类表（impl_record/test_plan/test_result/acceptance_plan/acceptance_result/spike_conclusion/bug_record/design_sync/product_features/topic_relations）的 schema、生成、校验、文档生成和手改指纹检测。表保存在 `.workflow_loop/records/<轮次编号>/` 下，是机器事实的唯一真本；正式文档由程序按表确定性生成。

### 门禁接入

- `src/workflow_loop/stages/stages.py`：impl/qa/acceptance_plan/topic_acceptance/spike/reproduce/update_code_design/spec 八个环节的第二道门在表启用时（表已填且非空）走表路径，先同步表→生成文档→检测手改→校验栏目和内容；空表不激活表路径，旧轮次走原有检查。
- `src/workflow_loop/test_execution.py`：`prepare_task`、`validate_prepared_tasks`、`missing_prepared_tasks` 在表模式下从测试计划表读取测试项和命令，一次登记全部任务。
- `src/workflow_loop/test_mapping.py`：`parse_test_plan_items` 中央表回退：表已填时从表构造计划项，空表视为纯人工主题。
- `src/workflow_loop/verification.py`：失效绑定改为盯表：表启用且已填时各阶段哈希对表文件计算；索引由主题关系表程序生成时退出文档绑定。
- `src/workflow_loop/artifact_validation.py`：测试计划文档校验和验收结果校验在表模式下短路为按表核对。
- `src/workflow_loop/traceability.py`：纯人工主题的追踪列写人工核对文案。
- `src/workflow_loop/cli.py`：新增 `workflow scaffold` 命令；`test prepare --from-tables` 按表登记；门禁校验异常统一转结构化失败（不再裸堆栈）；`main()` 顶层异常兜底；`cmd_status` 对已完成/已作废轮次给出一致方向；`cmd_abort` 删除本轮工作记录表。

### 核心设计约束

- 表是唯一真本，正式文档是其确定性生成结果；改内容只改表，手改文档被检出且不覆盖。
- 空表不激活表路径，保证旧轮次和未填表的环节完全兼容。
- 失效判定按表内容计算：相同表→相同文档→相同哈希，生成器不含时间戳。
- 主题关系表启用时，三类索引由程序按表重写，退出文档失效绑定。

### 产物

- `tests/test_records.py`：12 项行为测试覆盖表生成、校验分类、文档生成与手改检出、坏编码、status 修复、门禁异常兜底、损坏状态、索引生成、产品功能清单写回、穿刺/缺陷/设计同步表生成和生命周期。
- 本轮 qa 真实使用新路径：`workflow test prepare --from-tables` 从表一次登记 15 项，`workflow test run` 真实执行全部通过；结果表机器记录编号由程序回填。
