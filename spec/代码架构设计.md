...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：本轮主题验收结果为[当前仓库可由一条命令直接发布为新版本](../acceptance/当前仓库可由一条命令直接发布为新版本_验收结果.md)，使用机器记录 RUN-20260809T131840+0000-649e93d1、RUN-20260809T131840+0000-cd69105a、RUN-20260809T131840+0000-a5106a2f；最终全量回归记录为 REG-20260809T133254+0000-0b35d482；以上记录均来自当前状态并可复核。
- 产品与功能依据：[产品总说明](./产品总说明.md)及其链接的 17 份功能文档；本轮新增发布功能的产品行为与现有发布任务边界一致。
- 实施依据：[当前仓库可由一条命令直接发布为新版本_实施记录](../impl/当前仓库可由一条命令直接发布为新版本_实施记录.md)；真实实现为 `scripts/release.py` 及当前版本测试的派生断言。
- 主题测试依据：[当前仓库可由一条命令直接发布为新版本_测试结果](../qa/当前仓库可由一条命令直接发布为新版本_测试结果.md)；3 个登记目标均实际执行，执行数均大于零，跳过、失败和错误均为零。
- 主题验收依据：[当前仓库可由一条命令直接发布为新版本_验收结果](../acceptance/当前仓库可由一条命令直接发布为新版本_验收结果.md)；3 条验收条件均使用当前机器记录通过。
- 最终全量回归：执行入口为 `scripts/test_all.sh`；2026-08-09T13:32:54+00:00 开始，2026-08-09T13:33:27+00:00 结束；退出码为 0；337 项测试全部通过；机器记录为 REG-20260809T133254+0000-0b35d482；代码快照哈希为 `732e14a519985439bc65a2fa721d9f4cc1bf2abfc460d707b1c9d23415c7afe9`；输出哈希为 `e4bdc8674cdce9a2b172d1b0c130c2e49f31caff28f3fb63d38fa7a8b80dc9f0`。
- 整体验收：本轮用户已确认当前发布功能完成；程序已复核主题验收、最终回归和追踪前置条件有效。
- 全局追踪依据：[需求交付追踪表](../需求交付追踪表.md)。

本次实施保持第 4 章既有分层方向，在终端与发布适配层增加人工发布脚本，并复用现有标签发布任务。本文已经用当前真实代码映射和本轮测试、验收及回归记录替换实施前描述。
