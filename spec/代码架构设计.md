...
| 核对项 | 当前事实 | 依据 |
|---|---|---|
| 正式版本 | 源码、命令身份、安装脚本、Git 标签、GitHub 正式发布页和 PyPI（Python 公共软件包仓库）均为 `0.1.0`；以后公开新内容时使用未发布过的新版本号 | `pyproject.toml`、`src/workflow_loop/__init__.py`、`install.sh`、`install.ps1`、`.github/workflows/release.yml`、GitHub `v0.1.0` 正式发布页和 PyPI `workflow-loop 0.1.0` 页面 |
...
| 正式发布状态 | 标签 `v0.1.0` 指向提交 `f612e7d724ff34606919a5951545966c1efbcf1d`；GitHub Actions（GitHub 自动任务）运行 `30891477196` 全部成功，随后公开 PyPI `workflow-loop 0.1.0` 和名称为 `Workflow Loop 0.1.0` 的 GitHub 正式发布页及两个安装附件 | `.github/workflows/release.yml`、`tests/release_publication_checks.py`、GitHub 正式发布页和 PyPI 项目页面 |
...
| 最终交付 | 本轮主题验收、最终全量回归、整体验收和最终代码设计同步均已形成真实记录；正式收工在本阶段确认后执行 | [主题验收结果](../acceptance/无需开发任务按确认流程完成并保持边界_验收结果.md)、最终回归记录 `REG-20260806T063839+0000-92ca247a` 和本轮流水 |
| 本轮更新与卸载 | `workflow update`、`workflow uninstall`、`workflow uninstall --global` 和四个跨平台维护脚本均已实现；专题验收、跨平台发布任务和最终全量回归均已通过 | 本轮产品文档、本文第 6.14 和 6.15 节、三份专题验收结果和机器记录 `REG-20260805T063953+0000-fbd3a78f` |
...
2. 安装器检查 Python 3.11 或更高版本、现有命令身份和完整写入范围；用户确认后，安装当前正式版本的全局命令与项目骨架。本轮把安装身份从已发布的 `0.1.0` 更新为目标版本 `0.2.0`。
...
公开使用入口是仓库根目录的中文 `README.md`（项目说明文档）。当前已发布的 `0.1.0` 说明展示四个公开状态徽章，说明产品定位、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台正式安装命令、最小使用示例、简洁命令表、使用 `uv`（Python 项目管理工具）的源码开发方式、仓库结构和详细设计入口。本轮发布后，README 的当前安装入口和版本徽章指向 `0.2.0`；无需开发任务规则继续同步到安装器生成的 `AGENTS.md`（AI 契约文件）和 README。
...
| 产品规则 | 代码职责 | 实际承担位置 |
|---|---|---|
| 本轮版本一致、跨平台、事务安装 | 终端适配与共同安装事务分离；本轮所有安装身份从 `0.1.0` 同步为目标版本 `0.2.0` | `install.sh`、`install.ps1`、`src/workflow_loop/installer.py` |
...
`README.md`（项目说明文档）和 `LICENSE`（MIT 许可证文件）属于同一公开说明与发布适配层，不被运行时命令调用。`pyproject.toml`（Python 打包配置）提供 README、作者 `yuzyf`、MIT 许可证、固定项目简介和新仓库链接等分发元数据，并使说明与许可证进入构建产物。本轮发布流程将在 `v0.2.0`（版本 0.2.0 标签）上构建同一分发包，验证完成后再上传 PyPI（Python 公共软件包仓库），并创建名称、说明和六个维护脚本附件完整的 GitHub 正式发布页；已发布的 `v0.1.0` 保持不变。
...
- **对应产品内容**：公开产品身份、项目简介、作者、MIT 许可证和新仓库链接保持一致；本轮 GitHub 正式发布页与 PyPI（Python 公共软件包仓库）计划同时提供 `0.2.0`；README（项目说明文档）只展示真实可用的正式安装入口，已发布的 `0.1.0` 不覆盖或重发。
...
- **上游**：维护者先完成版本号、说明、许可证、元数据和发布配置，把最终提交推送到主分支，再让 `v0.2.0`（版本 0.2.0 标签）准确指向该提交；GitHub Actions（GitHub 自动任务）读取标签并启动发布流程。
...
- **状态和数据**：本轮发布流程使用 `v0.2.0`（版本 0.2.0 标签）、`0.2.0` 项目包版本、名称为 `Workflow Loop 0.2.0` 的正式发布、固定发布说明和六个维护脚本；成功结果是公开的 PyPI 版本和 GitHub 正式发布页。未来版本不由本轮代码自动调度。
- **失败结果**：版本、标签、可信发布身份、构建、测试或任一平台安装验证失败时，发布任务停止，不覆盖或重发已有版本。
- **验证位置**：`tests/test_release_workflow.py::test_release_gate_matrix_and_assets_are_structurally_complete` 核对触发条件、顺序、发布名称、说明和附件；`tests/test_public_project.py::test_built_distributions_include_readme_metadata_and_license` 核对 README、许可证、打包元数据和构建产物；`tests/public_repository_checks.py::test_local_and_public_repository_identity_are_canonical` 核对公开仓库和本地身份；`tests/release_publication_checks.py::test_pypi_and_github_release_contents_match` 核对两个线上发布渠道的实际内容。
...
- **验证位置**：`tests/test_light_task.py` 的 TC-01 至 TC-07 测试函数；正式机器记录为 `RUN-20260806T061844+0000-53211483`、`RUN-20260806T061844+0000-14270788`、`RUN-20260806T061845+0000-6a3e54e8`、`RUN-20260806T061846+0000-c300c6ea`、`RUN-20260806T061847+0000-38e1f2d0`、`RUN-20260806T061848+0000-3f5bb746`、`RUN-20260806T061849+0000-3a654c69`。
...
| 公开说明入口 | `src/workflow_loop/__init__.py::PRODUCT_IDENTITY`（命令版本身份）；`README.md`（根目录项目说明文档） | README 展示四个徽章、统一产品身份、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台命令、最小示例、简洁命令表、`uv`（Python 项目管理工具）源码开发方式、仓库结构和详细文档链接；实施后版本身份与安装命令中的 `0.2.0` 保持一致 | 文件缺失、结构不完整、出现旧仓库当前链接或命令版本不一致时，公开项目测试失败 | `tests/test_public_project.py::test_readme_has_the_confirmed_chinese_structure_and_entries`、`tests/test_light_task.py::test_installed_contract_and_readme_describe_the_light_task_rules` |
...
| 本轮 v0.2.0 标签发布 | `src/workflow_loop/__init__.py::PRODUCT_IDENTITY`（命令版本身份）；`.github/workflows/release.yml`（自动发布配置）的 `verify-and-test`（验证与测试任务）、build（构建任务）、`prepublish-smoke`（发布前安装验证任务）、`publish-pypi`（PyPI 发布任务）和 `github-release`（GitHub 正式发布任务） | 实施后只有 `v0.2.0` 标签进入公开发布路径；任务先完成版本核对、全量测试、构建和三平台四种环境验证，再上传 PyPI，最后创建名称为 `Workflow Loop 0.2.0`、说明完整且带六个维护脚本的 GitHub 正式发布页 | 任一前置任务失败时不继续后续发布；PyPI 已存在版本时停止，不覆盖或重发；手动任务只验证 | `tests/test_release_workflow.py::test_release_gate_matrix_and_assets_are_structurally_complete`；`tests/release_publication_checks.py::test_final_tag_workflow_succeeds`；`tests/release_publication_checks.py::test_pypi_and_github_release_contents_match` |
...
|---|---|---|---|---|---|
| 当前发布身份仍为 `0.1.0` | 本轮源码、命令身份、六个维护脚本、README、GitHub 标签、PyPI 包和 GitHub Release 统一使用 `0.2.0`；已发布的 `0.1.0` 保留且不可覆盖 | `pyproject.toml`、`src/workflow_loop/__init__.py`、六个维护脚本和 `.github/workflows/release.yml` 仍使用 `0.1.0`；代码层仍复用现有版本常量和发布任务，不需要新增模块 | 直接影响安装入口、更新入口、卸载入口、命令版本和公开发布触发条件 | 在实施环节同步版本常量、公开说明、六个脚本、发布流水线和相关测试；完成测试与验收后，再推送 `v0.2.0` 并核对 PyPI 与 GitHub Release | 代码确认；产品设计和用户确认已完成，尚未实施和验证 |

本轮不新增架构层。只修订终端、公开说明与发布适配层的版本身份和发布配置；命令编排、单轮状态、完整研发流程、无需开发任务、测试、验收和回退实现保持不变。上一轮已经完成的 `v0.1.0` 发布代码和记录保留，本轮实施将把同一发布链路的目标版本切换为 `v0.2.0`。
...
- 功能到代码映射：完整
- 未处理差异：暂无
- 核对依据：当前主题测试和最终回归机器记录编号为 RUN-20260806T061844+0000-53211483、RUN-20260806T061844+0000-14270788、RUN-20260806T061845+0000-6a3e54e8、RUN-20260806T061846+0000-c300c6ea、RUN-20260806T061847+0000-38e1f2d0、RUN-20260806T061848+0000-3f5bb746、RUN-20260806T061849+0000-3a654c69、REG-20260806T063839+0000-92ca247a；产品与功能文档、实施记录、主题测试结果、主题验收结果、整体验收记录和需求交付追踪表详见以下各项。
- 产品与功能依据：[产品总说明](./产品总说明.md)、[开始或继续一轮工作](./功能_开始或继续一轮工作.md)、[处理无需开发任务](./功能_处理无需开发任务.md)、[推进并确认工作环节](./功能_推进并确认工作环节.md)、[整轮作废并恢复](./功能_整轮作废并恢复.md)。
- 实施依据：[无需开发任务实施记录](../impl/无需开发任务按确认流程完成并保持边界_实施记录.md)，实际代码位于 `src/workflow_loop/path_composer.py`、`src/workflow_loop/state.py`、`src/workflow_loop/cli.py` 和 `src/workflow_loop/installer.py`，测试代码位于 `tests/test_light_task.py` 和 `tests/test_path_composer.py`。
- 主题测试依据：[无需开发任务测试结果](../qa/无需开发任务按确认流程完成并保持边界_测试结果.md)，全部机器测试记录编号为 RUN-20260806T061844+0000-53211483、RUN-20260806T061844+0000-14270788、RUN-20260806T061845+0000-6a3e54e8、RUN-20260806T061846+0000-c300c6ea、RUN-20260806T061847+0000-38e1f2d0、RUN-20260806T061848+0000-3f5bb746、RUN-20260806T061849+0000-3a654c69，七项均通过。
- 主题验收依据：[无需开发任务验收结果](../acceptance/无需开发任务按确认流程完成并保持边界_验收结果.md)，AC-01 至 AC-07 全部通过。
- 最终全量回归：执行入口为 `scripts/test_all.sh`；2026-08-06T06:38:39+00:00 开始，2026-08-06T06:39:04+00:00 结束；退出码为 0；193 项测试全部通过；机器记录为 REG-20260806T063839+0000-92ca247a；代码快照哈希为 `73120985125e0777c4e8ff746307c15a9bdd35194da9d8fbc45bc3658d3ab139`。
- 整体验收：2026-08-06T06:42:09+00:00 用户确认全部主题组合和最终全量回归满足本次需求。
- 全局追踪依据：[需求交付追踪表](../需求交付追踪表.md)。

本次实施保持第 4 章原有完整研发分层和依赖方向，同时新增无需开发任务的独立简单状态与命令分流；本文已经用最终真实代码映射和本轮测试、验收记录替换实施前描述。
