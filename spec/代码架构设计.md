...
| 核对项 | 当前事实 | 依据 |
|---|---|---|
| 当前版本身份 | 源码、命令身份、六个维护脚本、README、项目安装标记和自动发布配置统一为 `0.3.8` | `pyproject.toml`、`src/workflow_loop/__init__.py`、`install.sh`、`install.ps1`、`update.sh`、`update.ps1`、`uninstall.sh`、`uninstall.ps1`、`README.md`、`.workflow_loop/project.json`和 `.github/workflows/release.yml` |
...
| 最近保存的公开发布证据 | `0.2.0` 的标签、GitHub Actions 任务、PyPI 页面和 GitHub 正式发布页是已完成历史证据；当前 `0.3.8` 身份是现行源码和发布配置事实，不用旧任务编号冒充新版本已发布证明 | [0.2.0 主题测试结果](../qa/0_2_0_在_GitHub_和_PyPI_完成正式发布_测试结果.md)、`tests/release_publication_checks.py`和当前版本身份文件 |
...
| 最近已完成的交付证据 | 本轮填表与链接修复的九条验收条件、553 项最终全量回归和整体验收均已通过；本轮没有提交、推送或发布。 | [本轮主题验收结果](../acceptance/按程序指引填表和修链接都能一次过关_修复结果不被程序自己抹掉_验收结果.md)、[本轮测试结果](../qa/按程序指引填表和修链接都能一次过关_修复结果不被程序自己抹掉_测试结果.md)和本文第 9 节 |
...
2. 安装器检查 Python 3.11 或更高版本、现有命令身份和完整写入范围；用户确认后，安装 `0.3.8` 的全局命令与项目骨架。
...
公开使用入口是仓库根目录的中文 `README.md`（项目说明文档）。当前 README 展示四个公开状态徽章，说明产品定位、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台正式安装命令、最小使用示例、简洁命令表、使用 `uv`（Python 项目管理工具）的源码开发方式、仓库结构和详细设计入口。README 的安装入口和本地版本身份均为 `0.3.8`；无需开发任务规则同步到安装器生成的 `AGENTS.md`（AI 契约文件）和 README。
...
| 产品规则 | 代码职责 | 实际承担位置 |
|---|---|---|
| 当前版本一致、跨平台、事务安装 | 终端适配与共同安装事务分离；所有当前安装身份均为 `0.3.8` | `install.sh`、`install.ps1`、`src/workflow_loop/installer.py` |
...
| 公开说明、包元数据和当前版本发布 | 版本常量、静态说明、许可证、打包配置和发布任务共同承载 `0.3.8`；发布任务按 `v0.3.8`（版本 0.3.8 标签）先验证、再发布 PyPI、最后创建 GitHub 正式发布页 | `src/workflow_loop/__init__.py`（产品版本常量）、`README.md`（项目说明文档）、`LICENSE`（许可证文件）、`pyproject.toml`（Python 打包配置）、`.github/workflows/release.yml`（自动发布配置） |
...
`README.md`（项目说明文档）和 `LICENSE`（MIT 许可证文件）属于同一公开说明与发布适配层，不被运行时命令调用。`pyproject.toml`（Python 打包配置）提供 README、作者 `yuzyf`、MIT 许可证、固定项目简介和仓库链接等分发元数据，并使说明与许可证进入构建产物。当前版本身份为 `0.3.8`；自动发布层负责版本核对、全量测试、构建、三平台安装验证、PyPI 发布和 GitHub 正式发布页创建，历史发布证据留在对应测试与验收记录中。
...
- **对应产品内容**：公开产品身份、项目简介、作者、MIT 许可证和仓库链接保持一致；当前源码、README、安装脚本、分发元数据和发布任务统一使用 `0.3.8`，已发布版本不覆盖或重发。
...
- **上游**：维护者完成版本号、说明、许可证、元数据和发布配置，把最终提交推送到主分支，再让 `v0.3.8`（版本 0.3.8 标签）指向该提交；GitHub Actions（GitHub 自动任务）读取标签并启动发布流程。
...
- **下游**：用户从 README 复制当前系统的安装命令；安装脚本从 `v0.3.8` 正式发布页取得脚本，并从 PyPI 安装 `workflow-loop==0.3.8`（明确版本的 Python 分发包）。
- **状态和数据**：当前发布配置使用 `v0.3.8`（版本 0.3.8 标签）、`0.3.8` 项目包版本、名称为 `Workflow Loop 0.3.8` 的正式发布、固定发布说明和六个维护脚本。历史版本的提交、任务编号和公开页面证据留在对应测试与验收记录中。
...
产品要求见[功能_安装到项目.md](./功能_安装到项目.md)。当前安装使用现有 Python 3.11 或更高版本、目标版本 `0.3.8`、一次确认和完整事务；取消或失败不能留下半套项目。用户先从公开 README 了解产品和环境要求，再使用当前系统的正式安装入口。
...
| 骨架身份检查 | `src/workflow_loop/project.py::check_skeleton` | 核对产品版本、项目状态、两套资源仓库和智能体契约；当前产品版本为 `0.3.8` | 残缺或版本异常时写入前停止 | `tests/test_project.py::test_is_installed_rejects_project_state_without_complete_skeleton` |
...
| 公开说明入口 | `src/workflow_loop/__init__.py::PRODUCT_IDENTITY`（命令版本身份）；`README.md`（根目录项目说明文档） | README 展示四个徽章、统一产品身份、能力边界、四种工作意图、完整研发三道门和无需开发任务简单流程、环境要求、三平台命令、最小示例、简洁命令表、`uv`（Python 项目管理工具）源码开发方式、仓库结构和详细文档链接；版本身份与安装命令中的 `0.3.8` 保持一致 | 文件缺失、结构不完整、出现旧仓库当前链接或命令版本不一致时，公开项目测试失败 | `tests/test_public_project.py::test_readme_has_the_confirmed_chinese_structure_and_entries`、`tests/test_light_task.py::test_installed_contract_and_readme_describe_the_light_task_rules` |
...
| 复现文档校验 | `src/workflow_loop/artifact_validation.py::validate_reproduce_documents`（复现文档校验函数）与 `src/workflow_loop/records.py::validate_table`（工作记录表校验函数） | 新轮次使用版本 3：根因说明、根因位置、根因证据、运行环境和真实输入分栏填写，按表校验后生成，不再从正文拆标签。版本 1 和 2 保留原栏目及旧文档检查；工作流编号、主题、索引和章节仍核对。 | 新版缺少任一必需事实时指出具体栏目；根因中的换行或标签引用不被截断。确无技术不确定性只允许整栏一条“暂无”，其他必需事实中的占位内容仍拒绝。 | `tests/test_workflow_guidance.py::test_bug_facts_are_separate_and_literal`（独立事实及原文保留测试）；`tests/test_workflow_guidance.py::test_no_uncertainty_is_explicit_and_strict`（暂无与必需事实边界测试） |
...
| 缺陷记录锚点与手改保护 | `src/workflow_loop/records.py::_bug_defect_documents`、`_write_bug_documents`（缺陷文档写入函数）、`_existing_bug_result_blocks`（读取已有阶段结论块函数） | 八节缺陷正文使用统一标题定位；内部过程记录与正式缺陷文档各有固定出口。写前用独立生成凭据核对三个程序字段和正文摘要，不直接信任表内指纹；重新生成保留合法定位及第八节后续验收结论。 | 上游按章节锚点链接缺陷记录时链接检查通过；手改不被静默覆盖；重走缺陷复现不抹掉已发生的阶段事实 | `tests/test_workflow_guidance.py::test_program_fields_reject_tampering_before_writes`（全部表专用字段保护测试）；`tests/test_gate_rework_stability.py::test_bug_record_keeps_stage_result_blocks_on_regeneration`（再生保留验收结论测试） |
...
| 生成正式文档 | `src/workflow_loop/records.py::generate_document`（按表生成正文函数）、`src/workflow_loop/records.py::generate_test_result_document`（按机器记录生成测试结果函数）及 `src/workflow_loop/markdown_links.py::with_heading_anchors`（生成唯一标题定位函数） | 已连接出口的正式文档按表确定性生成，机器字段从当前记录回填；固定章节和条目共用唯一标题定位，合法历史大小写别名继续保留。产品功能小节、缺陷正式出口和上游引用回补也使用同一保护规则。 | 表文件无法解析时报结构化错误，不裸崩 | `tests/test_workflow_guidance.py::test_regeneration_preserves_anchors_and_manual_edits`（再生与回补保留定位测试） |
...
| 空表兼容 | `src/workflow_loop/records.py::workflow_uses_tables`（判断开工是否启用表流程的函数）和 `src/workflow_loop/records.py::table_is_filled`（检查是否已填事实的函数） | 是否启用表流程由开工冻结信息决定；已启用的轮次即使表为空也保持表流程并指出尚未填写，不能静默退回文档模式。没有启用工作记录表的历史轮次才沿用旧文档检查。 | 表缺失、损坏或未填时停留当前环节并报告；已填事实与旧轮次冻结版本不被重建或迁移。 | `tests/test_records.py::test_pristine_table_activates_table_path_and_reports_unfilled` |
...
本轮六项既定缺陷及九条验收条件内暂无未处理差异。范围外已确认问题继续保存在[待修复缺陷登记](../docs/待修复缺陷登记.md)：第 13 号计划路径异常、第 14 号发布版本文字滞后、第 15 号旧日志跨轮次恢复、第 16 号登记指引缺少选项、第 17 号最终同步表缺少正式架构生成出口。它们没有被写成本轮已修复；本轮架构事实先填工作记录表，再由一次性按表转换同步已有正文，未修改产品生成器。
