...
已引入工作记录表的轮次中，AI 在程序生成的 topic_relations 工作记录表里填写验收主题和前置关系，在 acceptance_plan 工作记录表里逐条填写验收条件（开始前状态、触发动作、可检查结果、通过标准、不通过标准、产品设计依据）；程序按表生成需求交付追踪表、acceptance/索引.md 和各主题验收计划。详见工作记录表与正式文档生成功能。
...
每个 `AC-xx` 的锚点由程序按表内“验收条件编号”生成（启用表时），文档模式手写时在标题前紧邻显式锚点 `<a id="ac-xx"></a>`，编号与标题一致，保证后续实施记录、测试计划和追踪表可以稳定导航到这一条验收条件。

启用工作记录表时，本阶段产物是验收计划表（acceptance_plan）：每条验收条件一行，填“验收条件编号、验收条件名称、开始前状态、触发动作、可检查结果、通过标准、不通过标准、产品设计依据”列；存在人工或混合验收条件时同时填“人工验收步骤”行清单（验收结果文档的人工小节由程序从这张表渲染）。正式文档由程序按表生成，AI 不手写文档正文。
