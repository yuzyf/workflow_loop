...
每个行为都必须使用上述可检查字段，不只针对删除、清空或覆盖。每条验收条件还要保留“条件与触发”和“预期结果”两项兼容摘要，但摘要不能代替开始前状态、触发动作、可检查结果、通过标准和不通过标准。
...
验收计划确认后的固定顺序是：实施计划与实施代码、测试计划、测试代码、正式主题测试。验收阶段引用尚未生成的下游文档时，只写反引号包裹的普通相对路径和“（待生成）”；文件存在后才写 Markdown 链接。
