...
1. 读取已经确认的 `spec/product.md`、其中链接的功能文档和 `spec/architecture_code_design.md`。
...
候选尚未被用户选择时，不写入 `spec/spike_index.md`，也不分配 `SP-001` 这类正式编号。
...
1. 创建 `spec/spike_index.md`。
2. 为每个已选项目分配 `SP-001`、`SP-002` 等编号。
3. 把每项穿刺状态先写为“待验证”。
4. 创建对应的 `spec/spike_<english-name>.md`，先填写第一至第四部分。
...
```text
.workflow_loop/spike_tmp/<english-name>/
```
...
结果状态只说明证据确认到了什么。“是否阻塞后续”单独说明当前能不能进入计划。
...
- 结果需要改变用户可见行为、功能范围或产品规则时，先由用户决定，再更新 `spec/product.md` 或对应功能文档。
- 结果影响接口、数据结构、模块、函数、调用过程、算法、性能限制、平台限制或异常处理时，更新 `spec/architecture_code_design.md`。
...
实现或修复办法仍未决定时，说明当前仍然阻塞，不能进入 `plan` 或 `fix_plan`。
...
用户一起检查穿刺清单、全部结论、剩余风险和更新后的产品设计、代码设计。执行门3时，程序必须重新校验当前文件；门2后文件被改坏时，清除旧通过状态并停留在 `spike`。重新校验通过后才能进入 `plan` 或 `fix_plan`，并清理 `.workflow_loop/spike_tmp/`、记录实际清理内容。
