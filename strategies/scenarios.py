...
4 个场景：
1. NewProjectScenario（场景 A）：完全实现，stages = [spec, spike, plan, acceptance, qa, impl, generate_code_design]
...
class NewProjectScenario(ScenarioStrategy):
    """新项目场景（场景 A）：spec → spike → plan → acceptance → qa → impl → generate_code_design。
    ...
    def stages(self) -> list:
        ...
        return [
            ...
            acceptanceStage(),            # acceptance标准
            QaStage(),             # 测试计划
            ImplStage(),           # 实施计划
            generate_code_designStage(),    # 生成代码架构设计文档
        ]

    def entry_instruction(self) -> str:
        # 进入这个场景时打印给 AI 的路线图
        # 告诉 AI 整体路线，不是只看眼前
        return "新项目入口：spec → spike → plan → acceptance → qa → impl → generate_code_design，7 个阶段顺序推进"


class ExistingNoWorkflowScenario(ScenarioStrategy):
    """存量无 workflow_loop 场景（场景 B）：
    code_design → spec → spike → plan → acceptance → qa → impl → update_code_design。
    ...
    def stages(self) -> list:
        ...
        # 应该是：[code_designStage(), SpecStage(), SpikeStage(), PlanStage(), acceptanceStage(), QaStage(), ImplStage(), update_code_designStage()]
        return []

    def entry_instruction(self) -> str:
        return "TODO: 存量无 workflow_loop 入口未实现（code_design → spec → spike → plan → acceptance → qa → impl → update_code_design）"


class BugfixScenario(ScenarioStrategy):
    """修 bug 场景（场景 C）：
    reproduce → fix_plan → acceptance → qa → impl → update_code_design。
    ...
    def stages(self) -> list:
        # TODO: 后面实现
        # 应该是：[ReproduceStage(), FixPlanStage(), acceptanceStage(), QaStage(), ImplStage(), update_code_designStage()]
        return []

    def entry_instruction(self) -> str:
        return "TODO: bugfix 子流程未实现（reproduce → fix_plan → acceptance → qa → impl → update_code_design）"


class ProductModScenario(ScenarioStrategy):
    """改产品设计场景（场景 D）：
    requirement → product_update → feature_split → spike → plan → acceptance → qa → impl → update_code_design。
    ...
    def stages(self) -> list:
        # TODO: 后面实现
        # 应该是：[RequirementStage(), ProductUpdateStage(), FeatureSplitStage(), SpikeStage(), PlanStage(), acceptanceStage(), QaStage(), ImplStage(), update_code_designStage()]
        return []

    def entry_instruction(self) -> str:
        return "TODO: product_mod 子流程未实现（requirement → product_update → feature_split → spike → plan → acceptance → qa → impl → update_code_design）"
